...
import { PORTAL_HOME_SECCIONES, type PortalHomeSeccion } from '@/lib/portal-home-sections';
import { mensajeSeguro, ERROR_RED } from '@/lib/errores';

// Clon de components/theme/home-editor.tsx (mismo patrón dnd-kit ya probado
// en producción), pero sobre `portalHome` en vez de `home`: aquí no hay
// ninguna sección "fija primero" que excluir — los 4 módulos son todos
// reordenables/ocultables (el saludo y la tarjeta grande de "próxima clase"
// ni siquiera entran en este sistema, quedan fuera del todo).

function Fila({ seccion, oculto, onToggle }: { seccion: PortalHomeSeccion; oculto: boolean; onToggle: () => void }) {
...
export function PortalHomeEditor() {
  ...
  const [items, setItems] = useState<string[]>(PORTAL_HOME_SECCIONES.map((s) => s.id));
  ...
  useEffect(() => {
    ...
    fetchLayout()
      .then((l) => {
        if (!vivo) return;
        const todos = PORTAL_HOME_SECCIONES.map((s) => s.id);
        const orden = [...l.portalHome.orden.filter((h) => todos.includes(h)), ...todos.filter((h) => !l.portalHome.orden.includes(h))];
        setItems(orden);
        setOcultos(new Set(l.portalHome.ocultos));
      })
      .catch(() => {})
      .finally(() => {
  ...
  async function guardar() {
    ...
    setAviso(null);
    try {
      await guardarLayoutApi({ portalHome: { orden: items, ocultos: [...ocultos] } });
      setAviso({ tipo: 'ok', texto: 'Inicio del portal guardado. Tus clientas lo verán la próxima vez que abran la app.' });
    } catch (e) {
  ...
  function restaurar() {
    setItems(PORTAL_HOME_SECCIONES.map((s) => s.id));
    setOcultos(new Set());
    setAviso(null);
  }

  const porId = new Map(PORTAL_HOME_SECCIONES.map((s) => [s.id, s]));

  return (
    <div className="space-y-4 max-w-xl">
      <p className="text-[13px] text-muted-foreground">
        Arrastra para reordenar los módulos de Inicio del portal de tus clientas y usa el ojo para ocultar los que no uses. El saludo y tu próxima clase se mantienen siempre arriba.
      </p>
      ...
      <div className="flex items-center gap-2 border-t border-border pt-4">
        ...
        <button onClick={guardar} disabled={guardando} className="text-[13px] font-bold px-4 py-2 rounded-xl bg-brand text-brand-foreground disabled:opacity-50">
          {guardando ? 'Guardando…' : 'Guardar Inicio del portal'}
        </button>
      </div>
    </div>
  );

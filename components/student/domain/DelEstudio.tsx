...
export function DelEstudio({ studioId, href }: { studioId: string; href: string }) {
  ...
  return (
    <Link href={href} className="card card--tap a-up" data-testid="del-estudio" style={{ display: 'block', padding: '13px 15px' }}>
      ...
      <p style={{ margin: '7px 0 0', fontSize: 'var(--t-body)', lineHeight: 1.5, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
        {post.tipo === 'EVENTO' && <span className="badge" style={{ marginRight: 6, verticalAlign: 'middle' }}>Evento</span>}
        {post.texto}
      </p>
      <p style={{ margin: '6px 0 0', fontSize: 'var(--t-micro)', fontWeight: 600, color: 'var(--subtle-foreground)' }}>{post.autorNombre} · {relativo(post.creadoEn)}</p>
    </Link>
  );
}

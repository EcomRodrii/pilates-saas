...
// Cambiar la contraseña desde dentro de la app. Lo usan la alumna
// (`/perfil/seguridad`) y la instructora (`/equipo/perfil/seguridad`): es la
// misma cuenta, así que es el mismo formulario.
//
...
export function CambiarPassword({ slug }: { slug: string }) {
  const { online } = useOnline();
  const { toast } = useToast();
  const { cambiarPassword } = useAuthStudent(slug);

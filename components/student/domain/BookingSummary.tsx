  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 9, background: 'var(--accent-soft)', borderRadius: 14, padding: '11px 14px', marginTop: 9 }}>
        ...
        <p style={{ margin: 0, fontSize: 12, fontWeight: 700, color: 'var(--accent-soft-foreground)' }}>{bono && quedan > 0 ? 'Se usará 1 sesión de tu ' + bono.nombre.toLowerCase() + ' (' + quedan + ' disponibles). No pagas nada hoy.' : 'Sin bono activo: clase suelta ' + euros(clase.precioSuelto) + '. El cobro se confirma en el siguiente paso.'}</p>
      </div>

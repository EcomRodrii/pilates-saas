...
export function MensajesCard({ studioId, nombreEstudio, href }: { studioId: string; nombreEstudio: string; href: string }) {
  ...
  return (
    <Link href={href} className="card card--tap a-up" data-testid="mensajes-card" style={{ display: 'block', padding: '13px 15px' }}>
      ...
      <p style={{ margin: '7px 0 0', fontSize: 'var(--t-body)', lineHeight: 1.5, fontWeight: conSinLeer.length > 0 ? 700 : 400, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
        {destacada ? (unaLinea(destacada.ultimo_cuerpo) || 'Sin mensajes todavía') : '¿Alguna duda? Escríbele al estudio.'}
      </p>
      {destacada && (
        <p style={{ margin: '6px 0 0', fontSize: 'var(--t-micro)', fontWeight: 600, color: 'var(--subtle-foreground)' }}>{unir(nombre, selloLista(destacada.ultimo_mensaje_en ?? destacada.creado_en))}</p>
      )}
    </Link>
  );
}

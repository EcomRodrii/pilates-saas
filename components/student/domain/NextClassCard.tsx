    ...
    <section aria-label="Tu próxima clase" className="a-pop" style={{ position: 'relative', borderRadius: 'var(--radius-hero)', overflow: 'hidden', boxShadow: 'var(--shadow-hero)', color: 'var(--accent-deep-foreground)' }}>
      ...
      <div style={{ position: 'relative', padding: '14px 15px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <p className="t-label" style={{ color: 'var(--accent-deep-muted)', display: 'flex', alignItems: 'center', gap: 6 }}><span aria-hidden style={{ width: 6, height: 6, borderRadius: 99, background: 'var(--accent-deep-muted)', animation: 'apPulse 2s infinite' }} />Tu próxima clase</p>
          <span className="t-num" style={{ fontSize: 'var(--t-meta)', fontWeight: 600, color: 'var(--accent-deep-muted)' }}>{etiquetaDia(clase.fecha).toLowerCase()} · {clase.hora}</span>
        </div>

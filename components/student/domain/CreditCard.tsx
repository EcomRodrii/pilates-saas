  ...
  return (
    <Link href={href('/bonos/' + bono.id)} className="card card--tap" style={{ display: 'block', padding: compacta ? '12px 15px' : '15px 17px', opacity: bono.estado === 'activo' ? 1 : .7 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10 }}>
        ...
        {compacta ? <span className="t-num" style={{ fontSize: 11.5, fontWeight: 700, color: 'var(--accent)' }}>quedan {quedan}</span> : <Badge tone={tono}>{etiqueta}</Badge>}
      </div>
      ...
      {!compacta && (
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 8 }}>
          ...
          <p className="t-meta t-num" style={{ fontSize: 11 }}>{bono.expiraEn ? (bono.estado === 'expirado' ? 'caducó ' : 'caduca ') + fechaCorta(bono.expiraEn) : 'sin caducidad'}</p>
        </div>
      )}
    </Link>
  );

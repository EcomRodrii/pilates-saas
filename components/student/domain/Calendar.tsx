        ...
        {Array.from({ length: dias }).map((_, i) => {
          ...
          return (
            <button key={iso} type="button" aria-pressed={sel} aria-label={iso + (tiene ? ', tienes clase' : '')} onClick={() => onChange(iso)}
              style={{ height: 44, border: esHoy && !sel ? '1.5px solid var(--primary)' : '1px solid transparent', borderRadius: 12, background: sel ? 'var(--primary)' : 'transparent', color: sel ? 'var(--primary-foreground)' : pasado ? 'var(--subtle-foreground)' : 'var(--foreground)', fontSize: 'var(--t-small)', fontWeight: sel || esHoy ? 800 : 600, position: 'relative', transition: 'all .2s' }}>
              {i + 1}

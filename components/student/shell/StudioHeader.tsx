  ...
  return (
    ...
    >
      <div style={{ maxWidth: 1040, margin: '0 auto', height: 56, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 18px' }}>
        <Link href={href()} aria-label={estudio.nombre} style={{ display: 'flex', alignItems: 'center', gap: 9, color: transparente ? '#FAF9F5' : 'var(--foreground)' }}>
          {estudio.logoUrl ? (
            ...
            <span style={{ width: 30, height: 30, borderRadius: 999, background: transparente ? 'rgba(250,249,245,.22)' : 'var(--accent)', color: transparente ? '#FAF9F5' : 'var(--accent-foreground)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11.5, fontWeight: 800 }}>
              {inicialDe(estudio.nombre)}
            </span>
          )}
          <span style={{ fontSize: 14, fontWeight: 800, letterSpacing: '-.01em' }}>{estudio.nombre}</span>
        </Link>

        <Link
          href={href('/notificaciones')}
          aria-label={'Notificaciones' + (noLeidas ? `, ${noLeidas} sin leer` : '')}
          style={{ position: 'relative', width: 40, height: 40, borderRadius: 999, display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid ' + (transparente ? 'rgba(255,255,255,.45)' : 'var(--border)'), background: transparente ? 'rgba(250,249,245,.22)' : 'var(--card)', color: transparente ? '#FAF9F5' : 'var(--foreground)' }}
        >

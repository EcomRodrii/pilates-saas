...
export function UnDia() {
  return (
    <section style={{ background: DARK, color: '#E8E8E4', padding: 'clamp(76px,9vw,124px) clamp(20px,4vw,44px)', position: 'relative', overflow: 'hidden' }}>
      <div style={{ position: 'absolute', top: '-8%', right: '-6%', width: 'min(680px,80vw)', height: 520, borderRadius: '50%', background: 'radial-gradient(circle, rgba(90,97,66,.18), transparent 66%)', pointerEvents: 'none' }} />
      <div className="tnt-wrap" style={{ maxWidth: 1280, margin: '0 auto', position: 'relative' }}>
        <div style={{ maxWidth: 700, marginBottom: 'clamp(40px,6vw,64px)' }}>
          <Eyebrow color="#A8B080">Un día con Tentare · ejemplo</Eyebrow>
...
function DayTimeline() {
  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      ...
      <div
        style={{ position: 'absolute', top: 8, left: 91, width: 2, height: `${pct * 100}%`, background: `linear-gradient(${ACC}, #191C11)`, boxShadow: '0 0 12px rgba(90,97,66,.55)', borderRadius: 2, transition: 'height .1s linear' }}
      ...
      {DAY_MOMENTS.map((m) => (
        <div key={m.t} className="tnt-moment" style={{ position: 'relative', display: 'grid', gridTemplateColumns: '72px 40px 1fr', alignItems: 'start', paddingBottom: 30 }}>
          ...
          <div style={{ position: 'relative', display: 'flex', justifyContent: 'center', paddingTop: 20 }}>
            <span style={{ width: 15, height: 15, borderRadius: '50%', background: DARK, border: `2px solid ${ACC}`, boxShadow: m.highlight ? '0 0 0 5px rgba(52,56,37,.22)' : '0 0 0 4px rgba(52,56,37,.12)' }} />
          </div>
          <Reveal
            style={
              m.highlight
                ? { background: 'linear-gradient(150deg,#14170E,#171717 70%)', border: '1px solid rgba(90,97,66,.5)', borderRadius: 18, padding: '20px 22px' }
                : m.good
                  ? { background: 'linear-gradient(135deg,#343825,#191C11)', borderRadius: 18, padding: '20px 22px', boxShadow: '0 30px 60px -30px rgba(52,56,37,.7)' }
          ...
          >
            ...
            <p style={{ fontSize: 15, lineHeight: 1.55, color: m.good ? '#E8EBDD' : '#A6A69E', margin: '0 0 14px' }}>{m.body}</p>
            <div className="lp-mono" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: m.good ? 'rgba(255,255,255,.14)' : m.highlight ? 'rgba(90,97,66,.16)' : 'rgba(90,97,66,.12)', border: m.good ? 'none' : '1px solid rgba(90,97,66,.22)', borderRadius: 999, padding: '7px 12px', fontSize: 11, color: m.good ? '#fff' : '#C2C9A8' }}>
              {m.tag}
            </div>
          </Reveal>
        </div>

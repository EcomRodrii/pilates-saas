...
// La demo del widget de reservas en la web de un estudio — la mitad «En tu web»
// de SeccionReservas (antes era su propia sección, SeccionWidget: se fusionó con
// la app de alumnas al aligerar la home, con un selector para ver una u otra).
//
    ...
    <>
      ...
      <style>{`
        ...
        .v5-navegador-body { display: grid; grid-template-columns: repeat(auto-fit,minmax(min(320px,100%),1fr));
          gap: clamp(24px,4vw,56px); padding: clamp(20px,2.4vw,32px) clamp(24px,4vw,56px); align-items: center; }
        ...
        .v5-w-fila, .v5-w-bono, .v5-w-reserva { display: flex; align-items: center; gap: 12px; border: 1px solid #EFEFEA;
          border-radius: 14px; padding: 10px 14px; transition: background .3s; }
        ...
        /* En estrecho, solo el widget: la barra de dirección ya dice que es la
           web del estudio, y la columna de la web ficticia doblaba el alto. */
        @media (max-width: 760px) {
          .v5-web { display: none; }
          .v5-navegador-body { padding: 14px; }
          .v5-widget { padding: 16px; }
        }
      `}</style>
    </>
  );

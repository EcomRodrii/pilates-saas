...
const CARDS = [
  { title: 'El sistema te dice qué hacer', body: 'El Centro de Control te pone delante lo que necesita tu atención. No buscas nada entre menús: llega solo.', color: '#A8B080' },
  ...
  { title: 'Migración incluida', body: 'Importas tus datos con asistentes guiados por CSV y te acompañamos en la puesta en marcha. No empiezas de cero ni te dejamos sola.', color: '#A8B080' },
  ...
  return (
    <section style={{ padding: 'clamp(56px,7vw,96px) clamp(20px,4vw,44px)' }}>
      <Reveal style={{ maxWidth: 1280, margin: '0 auto', background: DARK, color: '#E8E8E4', borderRadius: 28, padding: 'clamp(32px,5vw,60px)', position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', top: '-30%', right: '-6%', width: 440, height: 440, borderRadius: '50%', background: 'radial-gradient(circle, rgba(90,97,66,.28), transparent 64%)', pointerEvents: 'none' }} />
        <div style={{ position: 'relative' }}>
          <div style={{ maxWidth: 640, marginBottom: 'clamp(28px,4vw,40px)' }}>
            <div className="lp-mono" style={{ fontSize: 11.5, letterSpacing: '.16em', textTransform: 'uppercase', color: '#A8B080', marginBottom: 14 }}>Facilidad de uso, en serio</div>
          ...
          <div style={{ display: 'flex', gap: 'clamp(20px,4vw,40px)', flexWrap: 'wrap', marginBottom: 'clamp(28px,4vw,40px)', paddingBottom: 'clamp(28px,4vw,40px)', borderBottom: '1px solid rgba(255,255,255,.08)' }}>
            {STATS.map((s) => (
              <div key={s.label}>
                <div style={{ fontSize: 32, fontWeight: 800, letterSpacing: '-.03em', color: '#A8B080' }}>{s.value}</div>

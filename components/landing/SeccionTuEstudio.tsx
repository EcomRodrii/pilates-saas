...
export function SeccionTuEstudio() {
  ...
  const [pildora, setPildora] = useState<{ x: number; y: number; w: number; h: number } | null>(null);

  // La píldora oscura DESLIZA de una pestaña a otra en vez de saltar: dice de
  // dónde viene lo que cambia (continuidad). Se mide la pestaña activa —en el
  // móvil la tercera baja a otra fila, así que también cuenta el alto— y se
  // vuelve a medir si cambia el ancho. Hasta medir, la activa pinta su propio
  // fondo: sin JavaScript se ve igual.
  useLayoutEffect(() => {
    const el = lista.current;
    if (!el) return;
    const medir = () => {
      const activa = el.querySelector<HTMLElement>('[aria-selected="true"]');
      if (activa) setPildora({ x: activa.offsetLeft, y: activa.offsetTop, w: activa.offsetWidth, h: activa.offsetHeight });
    };
    medir();
    ...
    <section id="tu-estudio" className="v5-perfil" aria-labelledby="v5-perfil-h">
      <div className="v5-perfil-wrap">
        ...
        <div className="v5-perfil-caja lp-rv" style={{ ['--lp-r' as string]: 10 }}>
          <div ref={lista} className="v5-perfil-pestanas" role="tablist" aria-label="Tu situación" data-medido={pildora ? '' : undefined}>
            {pildora && (
              <span
                className="v5-perfil-pildora"
                aria-hidden
                style={{ transform: `translate(${pildora.x}px, ${pildora.y}px)`, width: pildora.w, height: pildora.h }}
              />
            )}
            {PERFILES.map((p) => (
              <button
          ...
          {PERFILES.map((p) => (
            ...
            >
              ...
              <Link href={p.enlace.href} className="v5-perfil-enlace lp-flecha">
      ...
      <style>{`
        ...
        .v5-perfil-pestanas { position: relative; display: flex; flex-wrap: wrap; gap: 4px; padding: 4px; border-radius: 999px;
          background: #F1F1EB; width: fit-content; max-width: 100%; }
        .v5-perfil-pildora { position: absolute; top: 0; left: 0; border-radius: 999px; background: #343825;
          pointer-events: none; box-shadow: 0 8px 18px -10px rgba(34,37,26,.6); }
        .v5-perfil-pestana { position: relative; z-index: 1; padding: 10px 18px; border: 0; border-radius: 999px; background: transparent; cursor: pointer;
          font: inherit; font-size: 14.5px; font-weight: 700; color: #4A4E3A; transition: color var(--motion-normal); }
        ...
        .v5-perfil-pestanas[data-medido] .v5-perfil-pestana-on { background: transparent; }
        ...
        @media (prefers-reduced-motion: no-preference) {
          .v5-perfil-panel:not([hidden]) { animation: v5-perfil-in var(--motion-slow) var(--motion-ease) both; }
          .v5-perfil-pestana { transition: color var(--motion-normal), transform var(--motion-fast) var(--motion-ease); }
          .v5-perfil-pildora { transition: transform var(--motion-slow) var(--motion-ease), width var(--motion-slow) var(--motion-ease),

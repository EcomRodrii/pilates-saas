    ...
    <li data-testid="tarjeta-horario" aria-label={nombre}
      ...
      <div className="min-w-0">
        <p className="text-sm font-semibold text-foreground flex items-center gap-1.5">
          ...
          <RefreshCw size={12} className="shrink-0 text-muted-foreground" aria-label="Se repite cada semana" />
        </p>
      ...
      <div>
        <div className="flex items-center gap-0.5" aria-hidden>
          {Array.from({ length: Math.min(t.aforo, PUNTOS_MAX) }, (_, i) => (
            <span key={i} className={cn('size-2 rounded-full', i < activas.length ? 'bg-primary' : 'bg-muted')} />
          ))}
        </div>
      ...
      <p className={cn('text-[11px]', fin.aviso ? 'font-medium text-warning' : 'text-muted-foreground')}>
        {fin.aviso && <AlertTriangle size={11} className="inline mr-1 -mt-0.5" aria-hidden />}
        {fin.texto}
      </p>
      ...
      <div className="flex flex-wrap gap-1.5">
        {p.puedeRenovar && (
          <button type="button" onClick={() => p.onRenovar(t)} aria-label={`Renovar ${nombre}`}
            className="flex items-center gap-1 px-2.5 py-1 rounded-lg text-[11px] font-bold border border-border text-foreground hover:bg-muted transition-colors">
        ...
        {p.puedeAsignarPlaza && (
          <button type="button" onClick={() => p.onAnadirPlaza(t)} aria-label={`Añadir plaza fija en ${nombre}`}
            className="flex items-center gap-1 px-2.5 py-1 rounded-lg text-[11px] font-bold border border-border text-foreground hover:bg-muted transition-colors">
            <Plus size={11} />Plaza fija
          </button>
        )}
        <button type="button" onClick={() => p.onVerClase(t)} aria-label={`Ver la próxima clase de ${nombre}`}
          className="flex items-center gap-1 px-2.5 py-1 rounded-lg text-[11px] font-bold text-muted-foreground hover:bg-muted transition-colors">
          <CalendarDays size={11} />Próxima clase
        </button>
      </div>
    </li>
  );

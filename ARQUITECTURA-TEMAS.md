...
| 6 | **Contraste por superficie** ✅ (`coloresDe()` en el render, no un componente `<Superficie>`: los bloques ya reciben `estilo`, así que un wrapper nuevo habría sido ceremonia) | Hoy solo validábamos al publicar. Esto hace que `EstiloBloque.fondo` no pueda romper la legibilidad, y el anidamiento sale gratis por cascada. |

...
### 9. El enlace mágico de reserva de una clienta crea también una sesión de STAFF, sin que nadie lo pida — CONFIRMADO en código, gravedad por acotar
...
**Impacto acotado (verificado, no supuesto):** entrar a `/equipo` con esta sesión contaminada no enseñó ningún dato real del estudio: como no hay fila en `usuarios` para mi email (confirmado por SQL: 0 filas), `app/(dashboard)/layout.tsx` nunca resuelve `studio` y la pantalla se queda en el esqueleto de carga para siempre — no llega a pintar clientas, cobros, ni nada sensible. La `usuarios`/RLS de servidor sigue siendo la barrera real, tal y como manda la convención de este repo.
**Lo que NO llegué a comprobar del todo:** si algún endpoint de API menos protegido confía en "hay sesión de Supabase" sin comprobar además `usuarios`/rol antes de devolver algo — solo miré el guardia del layout del panel, no cada ruta de `/api/*`. Por eso lo marco como "gravedad por acotar", no como crítico cerrado.
...
**Solución recomendada:** no tocar `detectSessionInUrl` a lo bruto. Las opciones reales: (a) que `lib/api-client.ts`/`lib/studio-context.tsx` usen un cliente de Supabase distinto (sin sesión, solo `anon`) para las llamadas públicas de `/reservar`, en vez del cliente de staff; o (b) que las páginas públicas (`/reservar`, `/portal/*`) nunca carguen el cliente de staff en absoluto, separando ese código en un chunk propio. Ambas tocan `lib/api-client.ts`, que se usa por todas partes — por eso no lo he tocado yo sin que se decida antes cuál de las dos vías se prefiere.
**Prioridad:** 🟠 Alta — recomiendo que lo revise el agente `tentare-seguridad` de este repo para acotar el alcance real (qué endpoints confían solo en "hay sesión") antes de decidir el arreglo.

...
//
//   node scripts/fotos-landing.mjs [carpeta-de-originales] [--permitir-ampliar]
//
...
import { FOTOS, CARPETA_PUBLICA, derivados } from '../components/landing/fotos.ts';
...
/** El mayor recorte de la proporción pedida, centrado en el foco y dentro de la foto. */
function zonaDeRecorte(ancho, alto, [pw, ph], foco) {
  let w = ancho;
  let h = Math.round((ancho * ph) / pw);
  if (h > alto) {
    h = alto;
    w = Math.round((alto * pw) / ph);
  }
  const dentro = (v, max) => Math.min(Math.max(v, 0), max);
  return {
    left: dentro(Math.round(foco.x * ancho - w / 2), ancho - w),
    top: dentro(Math.round(foco.y * alto - h / 2), alto - h),
    width: w,
    height: h,
  };
}
...
async function generar(clave, foto) {
  ...
  for (const d of derivados(foto)) {
    ...
    const zona = zonaDeRecorte(width, height, recorte.proporcion, recorte.foco);
...
for (const [clave, foto] of Object.entries(FOTOS)) {
  console.log(`\n${clave} → ${foto.id}`);
  for (const f of await generar(clave, foto)) esperados.add(f);
}

...
async function mockBackend(page: Page, opts: {
  ...
  // loginConPassword() (paso 'login' de vuelta con contraseña ya fijada
  // desde /acceso, ver lib/use-socia-session.ts) llama a Supabase Auth REST
  // directamente (cross-origin, igual que el mock de studios de arriba) —
  // sin este mock la E2E dependería de la Supabase real y del token falso
  // sembrado por seedSession, que no es un JWT válido.

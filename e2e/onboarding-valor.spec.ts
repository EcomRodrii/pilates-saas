...
test('la primera pantalla enseña valor antes de pedir un solo dato', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Tus alumnas se apuntan solas' })).toBeVisible({ timeout: 30_000 });
...
test('las cuatro pantallas avanzan y desembocan en el asistente', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'El bono se cobra solo' })).toBeVisible();
  await page.getByRole('button', { name: 'Siguiente' }).click();
  await expect(page.getByRole('heading', { name: 'La sustituta la buscamos nosotros' })).toBeVisible();
  await page.getByRole('button', { name: 'Siguiente' }).click();
  await expect(page.getByRole('heading', { name: 'Sabes qué clase sostiene el estudio' })).toBeVisible();
  ...
  await expect(page.getByRole('heading', { name: 'Tus alumnas se apuntan solas' })).toHaveCount(0);
});

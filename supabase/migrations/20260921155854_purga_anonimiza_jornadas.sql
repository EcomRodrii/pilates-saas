-- La purga de un estudio vencido conserva las jornadas de las instructoras y su
-- auditoría (registro de jornada), pero SIN datos que identifiquen a nadie: la
-- ficha ya se anonimizaba, y quedaban `created_by`/`edited_by` (la cuenta de Auth)
-- y el motivo de cada corrección, en texto libre. Ahora la cuenta pasa a un
-- centinela y el motivo se vacía. El resto de la función es la definición
-- vigente (20260921144341), sin tocar.
...
begin
  ...
  if position('''bajas_instructora''' in v_def) = 0
     ...
     or position('[''instructor_work_sessions'', ''created_by''' in v_def) = 0 then

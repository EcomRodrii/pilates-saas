-- `consentimientos_marketing_eventos` (recuperada en
-- 20260922004313_consentimiento_marketing_historial.sql) guarda `socio_id`:
-- el test de cobertura RGPD (`lib/socios/supresion-cobertura.test.ts`) exige
-- que toda tabla así esté clasificada en `lib/socios/supresion-clasificacion.ts`
-- Y, si es BORRAR/ANONIMIZAR, que `anonimizar_socio` la toque.
--
-- Clasificación: ANONIMIZAR. Es el historial de consentimiento de marketing
-- (DAR/RETIRAR) — mismo criterio que `consentimientos_salud_eventos` y
-- `aceptaciones_contrato_eventos`: se queda el hecho y la fecha como prueba de
-- que hubo consentimiento y cuándo se retiró, se anonimiza lo que identifica
-- el dispositivo/sesión concretos (`ip_hmac`, `user_agent`). A diferencia de
-- esas dos, no hay `firma` que sustituir: es un evento derivado de un trigger,
-- no una aceptación firmada.
--
-- Recreada idéntica a la vigente (20260915231920) más el bloque 8d — misma
-- firma, mismos grants y las mismas comprobaciones al aplicar. El test
-- `lib/socios/supresion-cobertura.test.ts` lee la ÚLTIMA migración que la
-- recrea, y exige que su cabecera documente TODA tabla clasificada. Fuente
-- documental: `lib/socios/supresion-clasificacion.ts`; copia para quien lea
-- solo el SQL:
--
...
-- | consentimientos_marketing_eventos | ANONIMIZAR | ⚠️ REVISIÓN LEGAL: ip_hmac/user_agent NULL; quedan acción (DAR/RETIRAR), fecha, origen y texto legal (20260922004313)
--
...
begin
  ...
  -- 8d. Historial de consentimiento de marketing (20260922004313). ⚠️ REVISIÓN LEGAL.
  -- Se queda el hecho (DAR/RETIRAR), la fecha y el texto legal como prueba;
  -- se anonimiza lo que identifica el dispositivo/sesión concreta.
  update public.consentimientos_marketing_eventos cme
     set ip_hmac = null, user_agent = null
   where cme.studio_id = p_studio_id and cme.socio_id = p_socio_id
     and (cme.ip_hmac is not null or cme.user_agent is not null);

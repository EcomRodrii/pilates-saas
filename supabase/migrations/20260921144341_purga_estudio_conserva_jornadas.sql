-- La purga de un estudio vencido declara qué hace con `instructor_work_sessions`
-- (cuelga de instructores): se CONSERVA. Es el registro de jornada del personal,
-- que el estudio está obligado a guardar; la ficha de la instructora sí se
-- anonimiza, así que las horas quedan sin datos identificables. Mismo criterio
-- que `liquidaciones_instructoras`. Solo cambia la lista de tablas conservadas
-- (que se informa en el recuento); el resto de la función es la definición
-- vigente de 20260914205418, sin tocar.
...
declare
  ...
  c_conservar constant text[] := array[
    ...
    'reservas', 'suscripciones', 'instructor_work_sessions'
  ];
  v_estudio     record;
  v_tabla       text;
  v_col         text[];

-- reservar_plaza nunca comprobaba que p_socio_id perteneciera de verdad a
-- p_studio_id: solo validaba p_studio_id contra current_studio_id() cuando
-- había un auth.uid() (protege al staff logueado de manipular el studio_id
-- de la llamada), pero no que el socio_id recibido fuera realmente de ESE
-- estudio. Probado en directo: se pudo crear una reserva con studio_id de
-- una sede y socio_id de OTRA sede distinta, sin ningún error — contamina
-- reservas cross-tenant (join roto: esa reserva referencia a una clienta
-- que no existe en el estudio al que dice pertenecer la reserva).

-- ─────────────────────────────────────────────────────────────────────────────
-- AUT-B (auditoría 24-sep): `campana_envios`, el registro por destinataria de una
-- campaña. Hasta hoy el único rastro de una campaña a 500 socias era un entero
-- (`campanas.enviados`): no se podía contestar «¿le llegó?», cruzar un rebote con
-- su envío ni reanudar una campaña de WhatsApp caída a mitad.
--
-- Estados: ENVIADO (con `provider_id` de Resend cuando es email), FALLIDO (con su
-- motivo) y OMITIDO (no se intentó: sin consentimiento vigente, buzón roto…).
-- UNIQUE (campana_id, socio_id): una destinataria, una fila por campaña, y un
-- reintento del step actualiza en vez de duplicar.
--
-- RGPD: clasificada ANONIMIZAR en lib/socios/supresion-clasificacion.ts. Por eso
-- se recrea `anonimizar_socio` (IDÉNTICA a la definición vigente, migr
-- 20260922013129, más el bloque `campana_envios`). La FK a `socios` con CASCADE
-- cubre además el borrado real (purga de estudio / eliminación dura).
--
...
create table if not exists public.campana_envios (
  ...
  constraint campana_envios_campana_socio_unq unique (campana_id, socio_id)
);
create index if not exists campana_envios_studio_idx on public.campana_envios (studio_id, campana_id);
create index if not exists campana_envios_provider_idx on public.campana_envios (provider_id) where provider_id is not null;

-- Solo servidor (lo escribe Inngest con service_role): RLS sin policies y sin
-- grants a los roles de cliente.

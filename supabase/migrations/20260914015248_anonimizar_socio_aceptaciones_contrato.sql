...
-- contrato. Se vacían la huella de IP, el user-agent, quién la introdujo en
-- mostrador y la cuenta que la registró; la firma tecleada pasa a
-- «[firma eliminada]» (la columna es NOT NULL).
--
-- La función se recrea IDÉNTICA a la definición vigente en producción
-- (20260913205144, leída con pg_get_functiondef el 2026-09-14) más el bloque
-- 8c, detrás de `to_regclass` y por EXECUTE (mismo patrón que 8b), para que no
-- falle en una base donde la tabla todavía no exista. Misma firma, mismos
-- grants (solo service_role) y las mismas comprobaciones al aplicar.
--
-- Fuente documental de la clasificación: `lib/socios/supresion-clasificacion.ts`
-- (el test `lib/socios/supresion-cobertura.test.ts` lee la ÚLTIMA migración que
-- recrea la función y falla si una tabla BORRAR/ANONIMIZAR no aparece aquí).
-- Copia en la cabecera para quien lea solo el SQL:
--

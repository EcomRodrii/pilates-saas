...
export async function generateMetadata({ params }: { params: Promise<{ ciudad: string; especialidad: string }> }): Promise<Metadata> {
  ...
  // Autorreferencia siempre (auditoría SEO 2026-09-10, hallazgo 1) — mismo
  // bug y mismo arreglo que la variante solo-ciudad: sin esto heredaba el
  // canonical por defecto del layout raíz.
  const url = `${LEGAL.url}/network/instructoras/ciudad/${ciudad}/${especialidad}`;
  return {

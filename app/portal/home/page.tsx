  ...
  return (
    <div className="bg-white min-h-full">
      ...
      <div
        ...
        style={{ background: 'linear-gradient(155deg, #15161B 0%, #3A2E9E 55%, #4B3FD6 100%)' }}
      >
        {/* Top row */}
        <div className="flex items-start justify-between mb-6">
          <div>
            <p className="text-[#C9C2FF] text-[13px] font-medium">{greeting}</p>
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            <div key={label} className="bg-white/10 rounded-2xl px-3 py-3 text-center">
              ...
              <p className="text-[#C9C2FF] text-[10px] font-semibold mt-1.5 uppercase tracking-wider">{label}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="px-4 -mt-4">
        {proxima ? (() => {
          ...
          <Link href="/portal/clases" className="block rounded-3xl overflow-hidden shadow-lg active:scale-[0.98] transition-transform">
            <div className="p-5 bg-gradient-to-br from-[#6355FF] to-[#4B3FD6] text-white">
              ...
              <div className="inline-flex items-center gap-2 bg-white text-[#6355FF] text-[13px] font-bold px-4 py-2.5 rounded-2xl">
                <Calendar size={15} />
        ...
        <div className="px-4 mt-5">
          <Link href="/portal/mi-plan" className={`block bg-white rounded-3xl shadow-sm border p-5 active:scale-[0.98] transition-transform ${bonoCaducado ? 'border-red-200' : 'border-black/[0.06]'}`}>
            <div className="flex items-center justify-between mb-4">
              <div>
                <p className="text-[11px] font-bold text-[#8A8B94] uppercase tracking-widest mb-1">Mi bono</p>
                <p className="text-[18px] font-extrabold text-[#15161B] leading-tight">{plan.nombre}</p>
              </div>
              <div className={`w-10 h-10 rounded-2xl flex items-center justify-center ${bonoCaducado ? 'bg-red-50' : 'bg-[#EEEBFF]'}`}>
                <CreditCard size={18} className={bonoCaducado ? 'text-red-500' : 'text-[#6355FF]'} />
              </div>
            </div>
            {bonoCaducado ? (
              ...
              <>
                <div className="flex justify-between items-baseline mb-2">
                  <span className="text-[22px] font-extrabold text-[#15161B]">{activeSus.sesionesRestantes}</span>
                  <span className="text-[13px] text-[#8A8B94]">de {plan.sesiones} sesiones</span>
                </div>
                <div className="h-2.5 bg-[#F1F1F6] rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      ...
                      backgroundColor: activeSus.sesionesRestantes > 3 ? '#6355FF' : '#EF4444',
                    }}
                  />
                </div>
              </>
              ...
              <div className="flex items-center justify-between">
                <p className="text-[14px] text-[#71727A]">
                  {activeSus.fechaFin
      ...
      {/* ── Acceso rápido ────────────────────────────── */}
      <div className="px-4 mt-5">
        <p className="text-[11px] font-bold text-[#8A8B94] uppercase tracking-widest mb-3">Acceso rápido</p>
        <div className="grid grid-cols-2 gap-3">
          {[
            { href: '/portal/clases', icon: Calendar, label: 'Reservar clase', color: '#6355FF', bg: '#EEEBFF' },
            ...
            { href: '/portal/progreso', icon: TrendingUp, label: 'Mi progreso', color: '#4B3FD6', bg: '#EEEBFF' },
            ...
            >
              ...
              <p className="text-[14px] font-bold text-[#15161B] leading-tight">{label}</p>
            </Link>
          ))}
        </div>
      </div>

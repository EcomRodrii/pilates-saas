...
const inputCls =
  'w-full rounded-xl border border-[#ECECF1] bg-white px-3.5 py-2.5 text-sm font-medium text-[#15161B] focus:outline-none focus:border-[#4B3FD6] transition-colors';
...
const BADGE: Record<string, { bg: string; text: string; label: string }> = {
  ...
  EN_CURSO:  { bg: '#DBEAFE', text: '#4B3FD6', label: 'En curso' },
};
...
export default function Pagos() {
  ...
  return (
    <div className="min-h-screen bg-[#F7F7FB] space-y-6 pb-10">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold text-[#15161B] tracking-tight">Pagos</h1>
          <p className="text-sm text-[#71727A] mt-0.5">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold border border-[#ECECF1] bg-white text-[#15161B] hover:bg-[#F7F7FB] transition-colors"
          >
            <FileText size={15} />
            Nueva factura
          </button>
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold text-white bg-[#15161B] hover:bg-[#2A2B34] transition-colors"
          >
            <Plus size={15} />
            Nuevo cobro
          </button>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white border border-[#ECECF1] rounded-xl p-5">
          <div className="flex items-center justify-between mb-3">
            <p className="text-xs font-semibold text-[#71727A] uppercase tracking-wider">
              Cobrado este mes
            </p>
          ...
          <p className="text-xs text-[#A2A3AC] mt-1">{monthLabel(thisMonth)}</p>
        </div>
        ...
        <div className="bg-white border border-[#ECECF1] rounded-xl p-5">
          ...
          <p className="text-xs text-[#A2A3AC] mt-1">{pendientesCount} recibo{pendientesCount !== 1 ? 's' : ''} pendiente{pendientesCount !== 1 ? 's' : ''}</p>
        </div>
        ...
        <div className="bg-white border border-[#ECECF1] rounded-xl p-5">
          ...
          <p className="text-xs text-[#A2A3AC] mt-1">miembro{kpis.sociosConDeuda !== 1 ? 's' : ''} con recibos pendientes</p>
        </div>
        ...
        <div className="bg-white border border-[#ECECF1] rounded-xl p-5">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="w-8 h-8 rounded-lg bg-[#EEEBFF] flex items-center justify-center">
              <BarChart3 size={15} className="text-[#4B3FD6]" />
            </div>
          </div>
          <p className="text-2xl font-extrabold text-[#15161B]">
            {kpis.mediaXSocia.toLocaleString('es-ES', { minimumFractionDigits: 2 })} €
          </p>
          <p className="text-xs text-[#A2A3AC] mt-1">sobre miembros activos este mes</p>
        </div>
      </div>

      {/* ── Main tabs ────────────────────────────────────────────────────────── */}
      <div className="flex items-center gap-1 border-b border-[#ECECF1] overflow-x-auto">
        {([
          ...
          <button
            ...
            className={cn(
              ...
              mainTab === t.value
                ...
                : 'border-transparent text-[#71727A] hover:text-[#15161B] hover:border-[#ECECF1]'
            )}
          >
            {t.label}
      ...
      {mainTab === 'cobros' && (
        <div className="space-y-4">
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      active
                        ...
                        : 'text-[#71727A] hover:text-[#15161B] hover:bg-white',
                    )}
                  >
                    {label}
                    <span className={cn(
                      'text-xs px-1.5 py-0.5 rounded-full font-bold',
                      active ? 'bg-white/20 text-white' : 'bg-[#F7F7FB] text-[#71727A]',
                    )}>
                      {count}
                    </span>
                  </button>
            ...
            <div className="flex flex-wrap items-center gap-3">
              <div className="relative flex-1 min-w-48">
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-[#ECECF1] bg-[#F7F7FB] pl-9 pr-3.5 py-2.5 text-sm text-[#15161B] focus:outline-none focus:border-[#4B3FD6] transition-colors"
                />
              </div>
              <div className="flex items-center gap-2">
                <label className="text-xs font-semibold text-[#71727A] whitespace-nowrap">Desde</label>
                <input type="month" value={desde} onChange={e => setDesde(e.target.value)}
                  className="rounded-xl border border-[#ECECF1] bg-white px-3 py-2 text-sm text-[#15161B] focus:outline-none focus:border-[#4B3FD6] transition-colors"
                />
              </div>
              <div className="flex items-center gap-2">
                <label className="text-xs font-semibold text-[#71727A] whitespace-nowrap">Hasta</label>
              ...
              <select
                ...
                className="rounded-xl border border-[#ECECF1] bg-white px-3 py-2 text-sm font-medium text-[#15161B] focus:outline-none focus:border-[#4B3FD6] transition-colors appearance-none cursor-pointer"
              >
                {SORT_OPTIONS.map(o => (
              ...
              <button
                onClick={exportCSV}
                className="flex items-center gap-1.5 px-3 py-2 rounded-xl border border-[#ECECF1] text-sm font-semibold text-[#71727A] hover:bg-[#F7F7FB] hover:text-[#15161B] transition-colors"
              >
                <Download size={14} />
                CSV
              </button>
          ...
          <div className="bg-white border border-[#ECECF1] rounded-xl overflow-hidden">
            <div className="px-5 py-3 border-b border-[#ECECF1] flex items-center justify-between">
              <p className="text-xs font-semibold text-[#71727A]">
            ...
            {filtradosCobros.length === 0 ? (
              <div className="py-16 text-center">
                <CreditCard size={32} className="text-[#ECECF1] mx-auto mb-3" />
                <p className="text-sm text-[#A2A3AC]">Sin recibos en esta categoría</p>
              </div>
            ) : (
              <div className="divide-y divide-[#F7F7FB]">
                {filtradosCobros.map(r => {
                  ...
                  return (
                    <div key={r.id}>
                      <div
                        className="flex items-center gap-4 px-5 py-4 hover:bg-[#F4F4F8] transition-colors group cursor-pointer"
                      ...
                      >
                        ...
                        <Link
                          ...
                          className="shrink-0"
                        >
                          <div className="w-9 h-9 rounded-full flex items-center justify-center text-xs font-bold bg-[#DBEAFE] text-[#4B3FD6]">
                            {initials}
                          </div>
                        </Link>
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-semibold text-[#15161B] truncate">{r.concepto}</p>
                          <p className="text-xs mt-0.5 text-[#A2A3AC] truncate">
                            <Link
                              href={`/socios/${r.socioId}`}
                              onClick={e => e.stopPropagation()}
                              className="hover:text-[#4B3FD6] hover:underline transition-colors"
                            >
                              {name}
                            </Link>
                            {' · '}
                        ...
                        <div className="text-right shrink-0 mr-2">
                          <p className="text-sm font-extrabold text-[#15161B]">
                        ...
                        >
                          {r.estado === 'PENDIENTE' && (
                            <>
                              ...
                              <button
                                ...
                                className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-bold bg-[#EEEBFF] text-[#4B3FD6] hover:bg-[#DBEAFE] transition-colors disabled:opacity-60"
                            ...
                            <>
                              {factura && (
                                <button
                                  ...
                                  className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-bold bg-[#F7F7FB] text-[#71727A] hover:bg-[#ECECF1] transition-colors"
                                  title="Ver factura"
                                >
                                  <FileText size={12} />
                          ...
                          {r.estado === 'DEVUELTO' && (
                            <button
                              ...
                              className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-bold bg-[#DBEAFE] text-[#4B3FD6] hover:bg-[#BFDBFE] transition-colors"
                            >
                              <RefreshCw size={12} />
                              Reintentar
                            </button>
                        ...
                        {/* Chevron */}
                        <div className="shrink-0 text-[#A2A3AC]">
                      ...
                      {/* Expanded detail */}
                      {expanded && (
                        <div className="px-5 pb-5 bg-[#F4F4F8] border-t border-[#ECECF1]">
                          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-4">
                            <div>
                              <p className="text-xs font-semibold text-[#A2A3AC] uppercase tracking-wider mb-1">Estado</p>
                            ...
                            <div>
                              <p className="text-xs font-semibold text-[#A2A3AC] uppercase tracking-wider mb-1">Vencimiento</p>
                              <p className="text-sm font-semibold text-[#15161B]">{fecha(r.fechaVencimiento)}</p>
                            </div>
                            {r.fechaCobro && (
                              <div>
                                <p className="text-xs font-semibold text-[#A2A3AC] uppercase tracking-wider mb-1">Fecha cobro</p>
                                <p className="text-sm font-semibold text-[#15161B]">{fecha(r.fechaCobro)}</p>
                              </div>
                            )}
                            {r.fechaDevolucion && (
                              <div>
                                <p className="text-xs font-semibold text-[#A2A3AC] uppercase tracking-wider mb-1">Devolución</p>
                                <p className="text-sm font-semibold text-[#15161B]">{fecha(r.fechaDevolucion)}</p>
                              </div>
                            )}
                            {typeof r.intentosReintento === 'number' && r.intentosReintento > 0 && (
                              <div>
                                <p className="text-xs font-semibold text-[#A2A3AC] uppercase tracking-wider mb-1">Reintentos</p>
                                <p className="text-sm font-semibold text-[#15161B]">{r.intentosReintento}</p>
                              </div>
                            )}
                            {sus && (
                              <div>
                                <p className="text-xs font-semibold text-[#A2A3AC] uppercase tracking-wider mb-1">Plan</p>
                                <p className="text-sm font-semibold text-[#15161B]">{planName(sus.planId)}</p>
                              </div>
                            )}
                            {factura && (
                              <div>
                                <p className="text-xs font-semibold text-[#A2A3AC] uppercase tracking-wider mb-1">Factura</p>
                                <p className="text-sm font-semibold text-[#4B3FD6]">{factura.numeroCompleto}</p>
                              </div>
                            )}
                          </div>
                        </div>
      ...
      {mainTab === 'suscripciones' && (
        <div className="bg-white border border-[#ECECF1] rounded-xl overflow-hidden">
          <div className="px-5 py-4 border-b border-[#ECECF1] flex items-center justify-between">
            <p className="text-sm font-semibold text-[#15161B]">
          ...
          {suscripciones.filter(s => s.estado === 'ACTIVA').length === 0 ? (
            <div className="py-16 text-center">
              <Users size={32} className="text-[#ECECF1] mx-auto mb-3" />
              <p className="text-sm text-[#A2A3AC]">No hay suscripciones activas</p>
            </div>
          ) : (
            <div className="overflow-x-auto">
              {/* Table header */}
              <div className="grid grid-cols-6 gap-4 px-5 py-2 bg-[#F4F4F8] border-b border-[#ECECF1] min-w-[700px]">
                {['Miembro', 'Plan', 'Precio/mes', 'Próximo cobro', 'Sesiones rest.', 'Acciones'].map(h => (
                  <p key={h} className="text-xs font-bold text-[#71727A] uppercase tracking-wider">{h}</p>
                ))}
              </div>
              ...
              <div className="divide-y divide-[#F7F7FB]">
                {suscripciones
                  ...
                  .map(sus => {
                    ...
                    return (
                      <div key={sus.id} className="grid grid-cols-6 gap-4 px-5 py-4 items-center hover:bg-[#F4F4F8] transition-colors group min-w-[700px]">
                        {/* Miembro */}
                        <div className="flex items-center gap-3 min-w-0">
                          <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold bg-[#DBEAFE] text-[#4B3FD6] shrink-0">
                            {initials}
                          </div>
                          <div className="min-w-0">
                            <Link href={`/socios/${sus.socioId}`}
                              className="text-sm font-semibold text-[#15161B] truncate hover:text-[#4B3FD6] hover:underline block">
                        ...
                        {/* Plan */}
                        <p className="text-sm text-[#15161B] font-medium truncate">{plan?.nombre ?? '—'}</p>

                        {/* Precio */}
                        <p className="text-sm font-bold text-[#15161B]">
                          {plan ? `${plan.precio.toLocaleString('es-ES', { minimumFractionDigits: 2 })} €` : '—'}
                        </p>

                        {/* Próximo cobro */}
                        <p className="text-sm text-[#71727A]">{fecha(nextCobro.toISOString())}</p>
                        ...
                        <div>
                          {sus.sesionesRestantes != null ? (
                            ...
                            <span className="text-xs text-[#A2A3AC]">Ilimitadas</span>
                          )}
                        </div>

                        {/* Acciones */}
                        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button className="text-xs px-2.5 py-1.5 rounded-lg font-bold border border-[#ECECF1] text-[#71727A] hover:bg-[#F7F7FB] transition-colors">
                            Cambiar plan
                          </button>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white border border-[#ECECF1] rounded-xl p-4">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div className="flex items-center gap-2">
                <label className="text-xs font-semibold text-[#71727A] whitespace-nowrap">Mes</label>
              ...
              <button
                ...
                className={cn(
                  ...
                  exportState === 'done'
                    ? 'bg-[#D1FAE5] text-[#059669]'
                    : 'border border-[#ECECF1] bg-white text-[#71727A] hover:bg-[#F7F7FB] hover:text-[#15161B]'
                )}
              >
          ...
          {historialAgrupado.length === 0 ? (
            <div className="bg-white border border-[#ECECF1] rounded-xl py-16 text-center">
              <BarChart3 size={32} className="text-[#ECECF1] mx-auto mb-3" />
              <p className="text-sm text-[#A2A3AC]">Sin resultados para los filtros seleccionados</p>
            </div>
          ) : (
            historialAgrupado.map(group => (
              <div key={group.ym} className="bg-white border border-[#ECECF1] rounded-xl overflow-hidden">
                {/* Month header */}
                <div className="flex items-center justify-between px-5 py-3 bg-[#F4F4F8] border-b border-[#ECECF1]">
                  <div className="flex items-center gap-2">
                    <Calendar size={14} className="text-[#71727A]" />
                    <p className="text-sm font-bold text-[#15161B] capitalize">{group.label}</p>
                    <span className="text-xs text-[#A2A3AC] font-medium">
                  ...
                  {group.items.map(r => {
                    ...
                    return (
                      <div key={r.id} className="flex items-center gap-4 px-5 py-3.5">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-semibold text-[#15161B] truncate">{r.concepto}</p>
                          <p className="text-xs text-[#A2A3AC] truncate">{socioName(r.socioId)}</p>
                        </div>
                        <p className="text-sm font-bold text-[#15161B] shrink-0">
                          {r.importe.toLocaleString('es-ES', { minimumFractionDigits: 2 })} €
                        </p>
                        <span
                        ...
                        <p className="text-xs text-[#A2A3AC] shrink-0 hidden sm:block">
      ...
      <Dialog open={showMasivo} onOpenChange={open => { if (!open && masivoProgress !== 'running') setShowMasivo(false); }}>
        <DialogContent className="max-w-lg max-h-[85vh] flex flex-col">
          <DialogHeader>
            <DialogTitle className="text-lg font-bold text-[#15161B] flex items-center gap-2">
          ...
          {masivoProgress === 'done' ? (
            ...
            <div className="flex flex-col items-center text-center gap-4 py-8">
              ...
              <div>
                <p className="text-lg font-bold text-[#15161B]">
                  {masivoTotal} cobro{masivoTotal !== 1 ? 's' : ''} procesado{masivoTotal !== 1 ? 's' : ''}
                </p>
                <p className="text-sm text-[#71727A] mt-1">
                  {masivoImporteTotal.toLocaleString('es-ES', { minimumFractionDigits: 2 })} € marcados como cobrados
                </p>
              </div>
              <button
                onClick={() => setShowMasivo(false)}
                className="px-6 py-2.5 rounded-xl text-sm font-bold text-white bg-[#15161B] hover:bg-[#2A2B34] transition-colors"
              >
                Cerrar
              </button>
            </div>
            ...
            <div className="flex flex-col items-center text-center gap-4 py-8">
              ...
              <div>
                ...
                <div className="w-48 h-2 bg-[#ECECF1] rounded-full mt-3 mx-auto overflow-hidden">
                  <div
            ...
            <>
              <div className="flex-1 overflow-y-auto space-y-2 my-2 pr-1">
                {masivoData.length === 0 ? (
                  <p className="text-sm text-[#A2A3AC] text-center py-8">No hay suscripciones activas</p>
                ) : (
                  masivoData.map(({ sus, socio, plan, pendientesRecibos }) => {
                    ...
                    return (
                        ...
                        className={cn(
                          ...
                          isSelected
                            ...
                            : hasPending
                              ...
                              : 'border-[#ECECF1] bg-white hover:bg-[#F4F4F8] opacity-60'
                        )}
                        onClick={() => {
                      ...
                      >
                        ...
                        {/* Info */}
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-semibold text-[#15161B] truncate">
                            {socio.nombre} {socio.apellidos}
                          </p>
                          <p className="text-xs text-[#A2A3AC] truncate">{plan?.nombre ?? '—'}</p>
                        </div>
              ...
              {/* Footer */}
              <div className="border-t border-[#ECECF1] pt-4 space-y-3">
                <div className="flex items-center justify-between text-sm">
                  ...
                  <p className="font-extrabold text-[#15161B]">
                ...
                <div className="flex gap-3">
                  <button
                    onClick={() => setShowMasivo(false)}
                    className="flex-1 py-2.5 rounded-xl text-sm font-bold border border-[#ECECF1] text-[#71727A] hover:bg-[#F7F7FB] transition-colors"
                  >
                    Cancelar
                  </button>
                  <button
      ...
      <Dialog open={showFactura} onOpenChange={open => { if (!open) setShowFactura(false); }}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle className="text-lg font-semibold text-[#15161B] flex items-center gap-2">
              <FileText size={18} />
              Nueva factura
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4 mt-2">
            ...
            {facturaForm.importe && !isNaN(parseFloat(facturaForm.importe)) && (
              <div className="bg-[#F4F4F8] border border-[#ECECF1] rounded-xl p-4 space-y-1.5">
                <div className="flex justify-between text-sm">
                  <span className="text-[#71727A]">Base imponible</span>
                  <span className="font-semibold text-[#15161B]">{parseFloat(facturaForm.importe).toFixed(2)} €</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-[#71727A]">IVA 21%</span>
                  <span className="font-semibold text-[#15161B]">{(parseFloat(facturaForm.importe) * 0.21).toFixed(2)} €</span>
                </div>
                <div className="flex justify-between text-sm font-bold border-t border-[#ECECF1] pt-1.5 mt-1.5">
                  <span className="text-[#15161B]">Total</span>
                  <span className="text-[#15161B]">{(parseFloat(facturaForm.importe) * 1.21).toFixed(2)} €</span>
                </div>
              </div>
            )}
          </div>
          <div className="flex gap-3 mt-6">
            ...
            <button
              ...
              className="flex-1 py-2.5 rounded-xl text-sm font-bold text-white bg-[#15161B] hover:bg-[#2A2B34] disabled:opacity-40 disabled:cursor-not-allowed transition-colors flex items-center justify-center gap-2"
            >
              <FileText size={14} />
              Generar factura
            </button>
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="flex gap-3 mt-6">
            ...
            <button
              ...
              className="flex-1 py-2.5 rounded-xl text-sm font-bold text-white bg-[#15161B] hover:bg-[#2A2B34] disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
            >
              Crear cobro
            </button>
          </div>
      ...
      <Dialog open={!!confirmEliminar} onOpenChange={open => !open && setConfirmEliminar(null)}>
        <DialogContent className="max-w-sm">
          <div className="flex flex-col items-center text-center gap-4 py-2">
            ...
            <div>
              <h3 className="text-base font-semibold text-[#15161B] mb-1">Eliminar recibo</h3>
              <p className="text-sm text-[#71727A]">Esta accion no se puede deshacer.</p>
            </div>

...
const inputCls =
  'w-full rounded-xl border border-[#F0E6E4] bg-white px-3.5 py-2.5 text-sm font-medium text-[#2B2429] focus:outline-none focus:border-[#A05A6B] transition-colors';
...
const BADGE: Record<string, { bg: string; text: string; label: string }> = {
  ...
  EN_CURSO:  { bg: '#DBEAFE', text: '#A05A6B', label: 'En curso' },
};
...
export default function Pagos() {
  ...
  return (
    <div className="min-h-screen bg-[#FDFBFA] space-y-6 pb-10">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold text-[#2B2429] tracking-tight">Pagos</h1>
          <p className="text-sm text-[#8B7D82] mt-0.5">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold border border-[#F0E6E4] bg-white text-[#2B2429] hover:bg-[#FDFBFA] transition-colors"
          >
            <FileText size={15} />
            Nueva factura
          </button>
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold text-white bg-[#2B2429] hover:bg-[#3A2F35] transition-colors"
          >
            <Plus size={15} />
            Nuevo cobro
          </button>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {/* Cobrado este mes */}
        <div className="bg-white border border-[#F0E6E4] rounded-xl p-5">
          <div className="flex items-center justify-between mb-3">
            <p className="text-xs font-semibold text-[#8B7D82] uppercase tracking-wider">
              Cobrado este mes
            </p>
          ...
          <p className="text-xs text-[#B0A3A8] mt-1">{monthLabel(thisMonth)}</p>
        </div>
        ...
        <div className="bg-white border border-[#F0E6E4] rounded-xl p-5">
          ...
          <p className="text-xs text-[#B0A3A8] mt-1">{pendientesCount} recibo{pendientesCount !== 1 ? 's' : ''} pendiente{pendientesCount !== 1 ? 's' : ''}</p>
        </div>
        ...
        <div className="bg-white border border-[#F0E6E4] rounded-xl p-5">
          ...
          <p className="text-xs text-[#B0A3A8] mt-1">miembro{kpis.sociosConDeuda !== 1 ? 's' : ''} con recibos pendientes</p>
        </div>
        ...
        <div className="bg-white border border-[#F0E6E4] rounded-xl p-5">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="w-8 h-8 rounded-lg bg-[#F7EDEF] flex items-center justify-center">
              <BarChart3 size={15} className="text-[#A05A6B]" />
            </div>
          </div>
          <p className="text-2xl font-extrabold text-[#2B2429]">
            {kpis.mediaXSocia.toLocaleString('es-ES', { minimumFractionDigits: 2 })} €
          </p>
          <p className="text-xs text-[#B0A3A8] mt-1">sobre miembros activos este mes</p>
        </div>
      </div>

      {/* ── Main tabs ────────────────────────────────────────────────────────── */}
      <div className="flex items-center gap-1 border-b border-[#F0E6E4] overflow-x-auto">
        {([
          ...
          <button
            ...
            className={cn(
              ...
              mainTab === t.value
                ...
                : 'border-transparent text-[#8B7D82] hover:text-[#2B2429] hover:border-[#F0E6E4]'
            )}
          >
            {t.label}
      ...
      {mainTab === 'cobros' && (
        <div className="space-y-4">
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      active
                        ...
                        : 'text-[#8B7D82] hover:text-[#2B2429] hover:bg-white',
                    )}
                  >
                    {label}
                    <span className={cn(
                      'text-xs px-1.5 py-0.5 rounded-full font-bold',
                      active ? 'bg-white/20 text-white' : 'bg-[#FDFBFA] text-[#8B7D82]',
                    )}>
                      {count}
                    </span>
                  </button>
            ...
            <div className="flex flex-wrap items-center gap-3">
              <div className="relative flex-1 min-w-48">
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-[#F0E6E4] bg-[#FDFBFA] pl-9 pr-3.5 py-2.5 text-sm text-[#2B2429] focus:outline-none focus:border-[#A05A6B] transition-colors"
                />
              </div>
              <div className="flex items-center gap-2">
                <label className="text-xs font-semibold text-[#8B7D82] whitespace-nowrap">Desde</label>
                <input type="month" value={desde} onChange={e => setDesde(e.target.value)}
                  className="rounded-xl border border-[#F0E6E4] bg-white px-3 py-2 text-sm text-[#2B2429] focus:outline-none focus:border-[#A05A6B] transition-colors"
                />
              </div>
              <div className="flex items-center gap-2">
                <label className="text-xs font-semibold text-[#8B7D82] whitespace-nowrap">Hasta</label>
              ...
              <select
                ...
                className="rounded-xl border border-[#F0E6E4] bg-white px-3 py-2 text-sm font-medium text-[#2B2429] focus:outline-none focus:border-[#A05A6B] transition-colors appearance-none cursor-pointer"
              >
                {SORT_OPTIONS.map(o => (
              ...
              <button
                onClick={exportCSV}
                className="flex items-center gap-1.5 px-3 py-2 rounded-xl border border-[#F0E6E4] text-sm font-semibold text-[#8B7D82] hover:bg-[#FDFBFA] hover:text-[#2B2429] transition-colors"
              >
                <Download size={14} />
                CSV
              </button>
          ...
          <div className="bg-white border border-[#F0E6E4] rounded-xl overflow-hidden">
            <div className="px-5 py-3 border-b border-[#F0E6E4] flex items-center justify-between">
              <p className="text-xs font-semibold text-[#8B7D82]">
            ...
            {filtradosCobros.length === 0 ? (
              <div className="py-16 text-center">
                <CreditCard size={32} className="text-[#F0E6E4] mx-auto mb-3" />
                <p className="text-sm text-[#B0A3A8]">Sin recibos en esta categoría</p>
              </div>
            ) : (
              <div className="divide-y divide-[#FDFBFA]">
                {filtradosCobros.map(r => {
                  ...
                  return (
                    <div key={r.id}>
                      <div
                        className="flex items-center gap-4 px-5 py-4 hover:bg-[#FAF6F4] transition-colors group cursor-pointer"
                      ...
                      >
                        ...
                        <Link
                          ...
                          className="shrink-0"
                        >
                          <div className="w-9 h-9 rounded-full flex items-center justify-center text-xs font-bold bg-[#DBEAFE] text-[#A05A6B]">
                            {initials}
                          </div>
                        </Link>
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-semibold text-[#2B2429] truncate">{r.concepto}</p>
                          <p className="text-xs mt-0.5 text-[#B0A3A8] truncate">
                            <Link
                              href={`/socios/${r.socioId}`}
                              onClick={e => e.stopPropagation()}
                              className="hover:text-[#A05A6B] hover:underline transition-colors"
                            >
                              {name}
                            </Link>
                            {' · '}
                        ...
                        <div className="text-right shrink-0 mr-2">
                          <p className="text-sm font-extrabold text-[#2B2429]">
                        ...
                        >
                          {r.estado === 'PENDIENTE' && (
                            <>
                              ...
                              <button
                                ...
                                className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-bold bg-[#F7EDEF] text-[#A05A6B] hover:bg-[#DBEAFE] transition-colors disabled:opacity-60"
                            ...
                            <>
                              {factura && (
                                <button
                                  ...
                                  className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-bold bg-[#FDFBFA] text-[#8B7D82] hover:bg-[#F0E6E4] transition-colors"
                                  title="Ver factura"
                                >
                                  <FileText size={12} />
                          ...
                          {r.estado === 'DEVUELTO' && (
                            <button
                              ...
                              className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-bold bg-[#DBEAFE] text-[#A05A6B] hover:bg-[#BFDBFE] transition-colors"
                            >
                              <RefreshCw size={12} />
                              Reintentar
                            </button>
                        ...
                        {/* Chevron */}
                        <div className="shrink-0 text-[#B0A3A8]">
                      ...
                      {/* Expanded detail */}
                      {expanded && (
                        <div className="px-5 pb-5 bg-[#FAF6F4] border-t border-[#F0E6E4]">
                          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-4">
                            <div>
                              <p className="text-xs font-semibold text-[#B0A3A8] uppercase tracking-wider mb-1">Estado</p>
                            ...
                            <div>
                              <p className="text-xs font-semibold text-[#B0A3A8] uppercase tracking-wider mb-1">Vencimiento</p>
                              <p className="text-sm font-semibold text-[#2B2429]">{fecha(r.fechaVencimiento)}</p>
                            </div>
                            {r.fechaCobro && (
                              <div>
                                <p className="text-xs font-semibold text-[#B0A3A8] uppercase tracking-wider mb-1">Fecha cobro</p>
                                <p className="text-sm font-semibold text-[#2B2429]">{fecha(r.fechaCobro)}</p>
                              </div>
                            )}
                            {r.fechaDevolucion && (
                              <div>
                                <p className="text-xs font-semibold text-[#B0A3A8] uppercase tracking-wider mb-1">Devolución</p>
                                <p className="text-sm font-semibold text-[#2B2429]">{fecha(r.fechaDevolucion)}</p>
                              </div>
                            )}
                            {typeof r.intentosReintento === 'number' && r.intentosReintento > 0 && (
                              <div>
                                <p className="text-xs font-semibold text-[#B0A3A8] uppercase tracking-wider mb-1">Reintentos</p>
                                <p className="text-sm font-semibold text-[#2B2429]">{r.intentosReintento}</p>
                              </div>
                            )}
                            {sus && (
                              <div>
                                <p className="text-xs font-semibold text-[#B0A3A8] uppercase tracking-wider mb-1">Plan</p>
                                <p className="text-sm font-semibold text-[#2B2429]">{planName(sus.planId)}</p>
                              </div>
                            )}
                            {factura && (
                              <div>
                                <p className="text-xs font-semibold text-[#B0A3A8] uppercase tracking-wider mb-1">Factura</p>
                                <p className="text-sm font-semibold text-[#A05A6B]">{factura.numeroCompleto}</p>
                              </div>
                            )}
                          </div>
                        </div>
      ...
      {mainTab === 'suscripciones' && (
        <div className="bg-white border border-[#F0E6E4] rounded-xl overflow-hidden">
          <div className="px-5 py-4 border-b border-[#F0E6E4] flex items-center justify-between">
            <p className="text-sm font-semibold text-[#2B2429]">
          ...
          {suscripciones.filter(s => s.estado === 'ACTIVA').length === 0 ? (
            <div className="py-16 text-center">
              <Users size={32} className="text-[#F0E6E4] mx-auto mb-3" />
              <p className="text-sm text-[#B0A3A8]">No hay suscripciones activas</p>
            </div>
          ) : (
            <div className="overflow-x-auto">
              {/* Table header */}
              <div className="grid grid-cols-6 gap-4 px-5 py-2 bg-[#FAF6F4] border-b border-[#F0E6E4] min-w-[700px]">
                {['Miembro', 'Plan', 'Precio/mes', 'Próximo cobro', 'Sesiones rest.', 'Acciones'].map(h => (
                  <p key={h} className="text-xs font-bold text-[#8B7D82] uppercase tracking-wider">{h}</p>
                ))}
              </div>
              ...
              <div className="divide-y divide-[#FDFBFA]">
                {suscripciones
                  ...
                  .map(sus => {
                    ...
                    return (
                      <div key={sus.id} className="grid grid-cols-6 gap-4 px-5 py-4 items-center hover:bg-[#FAF6F4] transition-colors group min-w-[700px]">
                        {/* Miembro */}
                        <div className="flex items-center gap-3 min-w-0">
                          <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold bg-[#DBEAFE] text-[#A05A6B] shrink-0">
                            {initials}
                          </div>
                          <div className="min-w-0">
                            <Link href={`/socios/${sus.socioId}`}
                              className="text-sm font-semibold text-[#2B2429] truncate hover:text-[#A05A6B] hover:underline block">
                        ...
                        {/* Plan */}
                        <p className="text-sm text-[#2B2429] font-medium truncate">{plan?.nombre ?? '—'}</p>

                        {/* Precio */}
                        <p className="text-sm font-bold text-[#2B2429]">
                          {plan ? `${plan.precio.toLocaleString('es-ES', { minimumFractionDigits: 2 })} €` : '—'}
                        </p>

                        {/* Próximo cobro */}
                        <p className="text-sm text-[#8B7D82]">{fecha(nextCobro.toISOString())}</p>
                        ...
                        <div>
                          {sus.sesionesRestantes != null ? (
                            ...
                            <span className="text-xs text-[#B0A3A8]">Ilimitadas</span>
                          )}
                        </div>

                        {/* Acciones */}
                        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button className="text-xs px-2.5 py-1.5 rounded-lg font-bold border border-[#F0E6E4] text-[#8B7D82] hover:bg-[#FDFBFA] transition-colors">
                            Cambiar plan
                          </button>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white border border-[#F0E6E4] rounded-xl p-4">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div className="flex items-center gap-2">
                <label className="text-xs font-semibold text-[#8B7D82] whitespace-nowrap">Mes</label>
              ...
              <button
                ...
                className={cn(
                  ...
                  exportState === 'done'
                    ? 'bg-[#D1FAE5] text-[#059669]'
                    : 'border border-[#F0E6E4] bg-white text-[#8B7D82] hover:bg-[#FDFBFA] hover:text-[#2B2429]'
                )}
              >
          ...
          {historialAgrupado.length === 0 ? (
            <div className="bg-white border border-[#F0E6E4] rounded-xl py-16 text-center">
              <BarChart3 size={32} className="text-[#F0E6E4] mx-auto mb-3" />
              <p className="text-sm text-[#B0A3A8]">Sin resultados para los filtros seleccionados</p>
            </div>
          ) : (
            historialAgrupado.map(group => (
              <div key={group.ym} className="bg-white border border-[#F0E6E4] rounded-xl overflow-hidden">
                {/* Month header */}
                <div className="flex items-center justify-between px-5 py-3 bg-[#FAF6F4] border-b border-[#F0E6E4]">
                  <div className="flex items-center gap-2">
                    <Calendar size={14} className="text-[#8B7D82]" />
                    <p className="text-sm font-bold text-[#2B2429] capitalize">{group.label}</p>
                    <span className="text-xs text-[#B0A3A8] font-medium">
                  ...
                  {group.items.map(r => {
                    ...
                    return (
                      <div key={r.id} className="flex items-center gap-4 px-5 py-3.5">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-semibold text-[#2B2429] truncate">{r.concepto}</p>
                          <p className="text-xs text-[#B0A3A8] truncate">{socioName(r.socioId)}</p>
                        </div>
                        <p className="text-sm font-bold text-[#2B2429] shrink-0">
                          {r.importe.toLocaleString('es-ES', { minimumFractionDigits: 2 })} €
                        </p>
                        <span
                        ...
                        <p className="text-xs text-[#B0A3A8] shrink-0 hidden sm:block">
      ...
      <Dialog open={showMasivo} onOpenChange={open => { if (!open && masivoProgress !== 'running') setShowMasivo(false); }}>
        <DialogContent className="max-w-lg max-h-[85vh] flex flex-col">
          <DialogHeader>
            <DialogTitle className="text-lg font-bold text-[#2B2429] flex items-center gap-2">
          ...
          {masivoProgress === 'done' ? (
            ...
            <div className="flex flex-col items-center text-center gap-4 py-8">
              ...
              <div>
                <p className="text-lg font-bold text-[#2B2429]">
              ...
              <button
                ...
                className="px-6 py-2.5 rounded-xl text-sm font-bold text-white bg-[#2B2429] hover:bg-[#3A2F35] transition-colors"
              >
                Cerrar
              </button>
            </div>
            ...
            <div className="flex flex-col items-center text-center gap-4 py-8">
              ...
              <div>
                ...
                <div className="w-48 h-2 bg-[#F0E6E4] rounded-full mt-3 mx-auto overflow-hidden">
                  <div
            ...
            <>
              <div className="flex-1 overflow-y-auto space-y-2 my-2 pr-1">
                {masivoData.length === 0 ? (
                  <p className="text-sm text-[#B0A3A8] text-center py-8">No hay suscripciones activas</p>
                ) : (
                  masivoData.map(({ sus, socio, plan, pendientesRecibos }) => {
                    ...
                    return (
                        ...
                        className={cn(
                          ...
                          isSelected
                            ...
                            : hasPending
                              ...
                              : 'border-[#F0E6E4] bg-white hover:bg-[#FAF6F4] opacity-60'
                        )}
                        onClick={() => {
                      ...
                      >
                        ...
                        {/* Info */}
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-semibold text-[#2B2429] truncate">
                            {socio.nombre} {socio.apellidos}
                          </p>
                          <p className="text-xs text-[#B0A3A8] truncate">{plan?.nombre ?? '—'}</p>
                        </div>
              ...
              {/* Footer */}
              <div className="border-t border-[#F0E6E4] pt-4 space-y-3">
                <div className="flex items-center justify-between text-sm">
                  <p className="text-[#8B7D82]">
                    {masivoSelected.size} recibo{masivoSelected.size !== 1 ? 's' : ''} seleccionado{masivoSelected.size !== 1 ? 's' : ''}
                  </p>
                  <p className="font-extrabold text-[#2B2429]">
                ...
                <div className="flex gap-3">
                  <button
                    onClick={() => setShowMasivo(false)}
                    className="flex-1 py-2.5 rounded-xl text-sm font-bold border border-[#F0E6E4] text-[#8B7D82] hover:bg-[#FDFBFA] transition-colors"
                  >
                    Cancelar
                  </button>
                  <button
      ...
      <Dialog open={showFactura} onOpenChange={open => { if (!open) setShowFactura(false); }}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle className="text-lg font-semibold text-[#2B2429] flex items-center gap-2">
              <FileText size={18} />
              Nueva factura
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4 mt-2">
            ...
            {facturaForm.importe && !isNaN(parseFloat(facturaForm.importe)) && (
              <div className="bg-[#FAF6F4] border border-[#F0E6E4] rounded-xl p-4 space-y-1.5">
                <div className="flex justify-between text-sm">
                  <span className="text-[#8B7D82]">Base imponible</span>
                  <span className="font-semibold text-[#2B2429]">{parseFloat(facturaForm.importe).toFixed(2)} €</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-[#8B7D82]">IVA 21%</span>
                  <span className="font-semibold text-[#2B2429]">{(parseFloat(facturaForm.importe) * 0.21).toFixed(2)} €</span>
                </div>
                <div className="flex justify-between text-sm font-bold border-t border-[#F0E6E4] pt-1.5 mt-1.5">
                  <span className="text-[#2B2429]">Total</span>
                  <span className="text-[#2B2429]">{(parseFloat(facturaForm.importe) * 1.21).toFixed(2)} €</span>
                </div>
              </div>
            )}
          </div>
          <div className="flex gap-3 mt-6">
            ...
            <button
              ...
              className="flex-1 py-2.5 rounded-xl text-sm font-bold text-white bg-[#2B2429] hover:bg-[#3A2F35] disabled:opacity-40 disabled:cursor-not-allowed transition-colors flex items-center justify-center gap-2"
            >
              <FileText size={14} />
              Generar factura
            </button>
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="flex gap-3 mt-6">
            ...
            <button
              ...
              className="flex-1 py-2.5 rounded-xl text-sm font-bold text-white bg-[#2B2429] hover:bg-[#3A2F35] disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
            >
              Crear cobro
            </button>
          </div>
      ...
      <Dialog open={!!confirmEliminar} onOpenChange={open => !open && setConfirmEliminar(null)}>
        <DialogContent className="max-w-sm">
          <div className="flex flex-col items-center text-center gap-4 py-2">
            ...
            <div>
              <h3 className="text-base font-semibold text-[#2B2429] mb-1">Eliminar recibo</h3>
              <p className="text-sm text-[#8B7D82]">Esta accion no se puede deshacer.</p>
            </div>

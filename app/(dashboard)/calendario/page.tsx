...
const inputCls = 'w-full rounded-xl border border-[#ECECF1] bg-white px-3.5 py-2.5 text-sm font-medium text-[#15161B] focus:outline-none focus:border-[#A2A3AC] transition-colors';
const selectCls = 'w-full rounded-xl border border-[#ECECF1] bg-white px-3.5 py-2.5 text-sm font-medium text-[#15161B] focus:outline-none focus:border-[#A2A3AC] transition-colors appearance-none';
  ...
  return (
    <button
      ...
      className={cn(
        ...
        active ? 'bg-[#15161B] text-white' : 'bg-[#F1F1F6] text-[#71727A] hover:bg-[#ECECF1]'
      )}
    >
      {label}
    </button>
...
function StatsBar({ sesiones, reservas, todayStr }: {
  ...
  const stats = [
    { icon: CalendarDays, label: 'Clases hoy', value: hoy.length, sub: hoy.length === 1 ? 'sesión' : 'sesiones', color: '#6355FF', bg: '#EEEBFF' },
    { icon: Users, label: 'Plazas totales', value: totalPlazas, sub: 'disponibles hoy', color: '#15161B', bg: '#F1F5F9' },
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
      {stats.map(({ icon: Icon, label, value, sub, color, bg }) => (
        <div key={label} className="bg-white border border-[#ECECF1] rounded-2xl p-4 flex flex-col gap-2.5 hover:shadow-sm transition-shadow">
          ...
          <div>
            <p className="text-[26px] font-extrabold text-[#15161B] leading-none tracking-tight tabular-nums">{value}</p>
            <p className="text-[11px] text-[#A2A3AC] mt-1 truncate">{sub}</p>
          </div>
        </div>
      ))}
    </div>
  ...
  return (
    <div className="flex items-center gap-2 flex-wrap">
      <div className="relative flex-1 min-w-[160px] max-w-xs">
        <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#A2A3AC]" />
        <input
          className="w-full rounded-xl border border-[#ECECF1] bg-white pl-8 pr-3 py-2 text-sm font-medium text-[#15161B] focus:outline-none focus:border-[#A2A3AC] transition-colors placeholder:text-[#A2A3AC]"
      ...
      <select
        className="rounded-xl border border-[#ECECF1] bg-white px-3 py-2 text-sm font-medium text-[#15161B] focus:outline-none appearance-none cursor-pointer"
      ...
      {hayFiltros && (
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold border border-[#ECECF1] text-[#71727A] hover:bg-gray-50 transition-colors"
        >
          <X size={12} />Limpiar
        </button>
      )}
...
function SessionSidebar({
  ...
  return (
    <div className="w-full lg:w-[380px] shrink-0 bg-white lg:border-l border-[#ECECF1] flex flex-col h-full overflow-hidden">
      {/* Header with class color accent */}
      <div
        className="px-5 pt-4 pb-3 border-b border-[#ECECF1]"
      ...
      >
        <div className="flex items-start justify-between mb-2">
          <div className="flex items-center gap-2">
            ...
            />
            <h2 className="text-base font-bold text-[#15161B] leading-tight">{sesion.tipoClase.nombre}</h2>
          </div>
          <button
            onClick={onClose}
            className="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-[#F1F1F6] text-[#71727A] transition-colors ml-2 shrink-0"
          >
            <X size={15} />
          </button>
        </div>

        <p className="text-xs font-semibold text-[#71727A] capitalize mb-3">{fechaLabel}</p>

        <div className="flex items-center gap-3 text-xs font-medium text-[#71727A] flex-wrap">
        ...
        <div className="mt-3 space-y-1">
          <div className="flex justify-between text-xs font-semibold">
            <span className="text-[#71727A]">Ocupación</span>
      ...
      {/* Action buttons */}
      <div className="px-5 py-2.5 border-b border-[#ECECF1] flex items-center gap-2">
        <button
          onClick={onOpenEdit}
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold border border-[#ECECF1] text-[#15161B] hover:bg-gray-50 transition-colors"
        >
          <Pencil size={12} />Editar
        </button>
        <button
          onClick={() => setShowConfirm('cancelar')}
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold border border-[#ECECF1] text-[#71727A] hover:bg-gray-50 transition-colors"
        >
          <X size={12} />Cancelar
        </button>
        <button
      ...
      {showConfirm && (
        <div className="absolute inset-0 bg-white z-30 flex flex-col items-center justify-center gap-5 p-8 text-center">
          ...
          <div>
            <h3 className="text-base font-bold text-[#15161B] mb-1">
          ...
          <div className="flex gap-3 w-full">
            <button
              ...
              className="flex-1 py-2.5 rounded-xl text-sm font-bold border border-[#ECECF1] text-[#71727A] hover:bg-gray-50"
            >
              Volver
            </button>
            <button
      ...
      {/* Tabs */}
      <div className="flex border-b border-[#ECECF1] px-5 pt-2">
        {(['asistentes', 'mapa'] as const).map(tab => (
          <button
            ...
            className={cn(
              ...
              activeTab === tab
                ? 'border-[#15161B] text-[#15161B]'
                : 'border-transparent text-[#A2A3AC] hover:text-[#71727A]'
            )}
          >
        ...
        {activeTab === 'asistentes' ? (
          <div className="p-4 space-y-1">
            ...
            {!showAnadir ? (
              <button
                ...
                className="w-full flex items-center gap-2 py-2.5 px-3 rounded-xl border border-dashed border-[#ECECF1] text-xs font-bold text-[#71727A] hover:border-[#A2A3AC] hover:text-[#15161B] transition-colors mb-3"
              >
              ...
              <div className="mb-3 space-y-2">
                <input
                  className="w-full rounded-xl border border-[#ECECF1] bg-white px-3.5 py-2.5 text-sm font-medium text-[#15161B] focus:outline-none focus:border-[#A2A3AC]"
                ...
                <div className="space-y-0.5 max-h-40 overflow-y-auto">
                  {sociosDisponibles.slice(0, 8).map(s => (
                    <button
                      ...
                      className="w-full flex items-center gap-2.5 py-2 px-3 rounded-lg hover:bg-[#F4F4F8] transition-colors text-left"
                    >
                      <div className="w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-bold shrink-0" style={{ backgroundColor: '#EEEBFF', color: '#6355FF' }}>
                        {s.nombre[0]}{s.apellidos[0]}
                      </div>
                      <div className="min-w-0">
                        <p className="text-xs font-semibold text-[#15161B] truncate">{s.nombre} {s.apellidos}</p>
                        <p className="text-[10px] text-[#A2A3AC] truncate">{s.email}</p>
                      </div>
                    </button>
                  ))}
                  {sociosDisponibles.length === 0 && (
                    <p className="text-xs text-center py-3 text-[#A2A3AC]">No hay socias disponibles</p>
                  )}
                </div>
                <button
                  onClick={() => { setShowAnadir(false); setBuscarSocia(''); }}
                  className="w-full py-1.5 rounded-lg text-xs font-semibold text-[#A2A3AC] hover:bg-gray-50"
                >
                  Cancelar
                </button>
              </div>
            ...
            {reservas.length === 0 ? (
              <div className="py-8 text-center">
                <p className="text-sm text-[#A2A3AC]">Sin asistentes aún</p>
              </div>
            ) : (
              reservas
                ...
                .map(r => (
                  <div
                    key={r.id}
                    className="flex items-center gap-2.5 py-2 px-2.5 rounded-xl hover:bg-[#F4F4F8] group transition-colors"
                  >
                    <div
                      ...
                      style={
                        r.estado === 'ASISTIDA'
                          ...
                          : { backgroundColor: '#EEEBFF', color: '#6355FF' }
                      }
                    >
                      {r.socio?.nombre[0]}{r.socio?.apellidos[0]}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-semibold text-[#15161B] truncate">{r.socio?.nombre} {r.socio?.apellidos}</p>
                      <p className="text-[10px] text-[#A2A3AC]">
                        {r.estado === 'LISTA_ESPERA'
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      ...
                      <button
                        ...
                        className="w-6 h-6 flex items-center justify-center rounded-lg text-[#C7C8D0] hover:bg-red-50 hover:text-red-400 transition-colors opacity-0 group-hover:opacity-100"
                      >
                        <X size={12} />
                      </button>
                    </div>
          ...
          <div className="p-4">
            {spotsActuales.length > 0 ? (
              ...
              <div className="flex items-center justify-center h-32 rounded-2xl border-2 border-dashed border-[#ECECF1] text-sm text-[#A2A3AC]">
      ...
      {/* Footer: kiosk link */}
      <div className="px-5 py-3 border-t border-[#ECECF1]">
        <a
          href={`/kiosk/${sesion.id}`}
          className="flex items-center gap-1.5 text-xs font-semibold text-[#71727A] hover:text-[#15161B] transition-colors"
        >
...
function ModalClasesRecurrentes({
  ...
  const f2 = 'w-full border border-[#ECECF1] rounded-xl px-3.5 py-2.5 text-sm focus:border-[#15161B] focus:outline-none text-[#15161B]';
  const s2 = 'w-full border border-[#ECECF1] rounded-xl px-3.5 py-2.5 text-sm focus:border-[#15161B] focus:outline-none text-[#15161B] bg-white appearance-none';

  return (
    <Dialog open={open} onOpenChange={o => !o && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-lg font-semibold text-[#15161B]">Crear clases recurrentes</DialogTitle>
          <p className="text-sm text-[#71727A] mt-0.5">Genera múltiples sesiones de una vez</p>
        </DialogHeader>
        <div className="space-y-4 mt-3">
          <div className="space-y-1.5">
            <label className="text-xs font-bold text-[#15161B] uppercase tracking-wider">Tipo de clase</label>
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-bold text-[#15161B] uppercase tracking-wider">Instructora</label>
            <select className={s2} value={form.instructorId} onChange={e => setForm(f => ({ ...f, instructorId: e.target.value }))}>
              {instructores.map(i => <option key={i.id} value={i.id}>{i.nombre}</option>)}
            </select>
          </div>
          <div className="space-y-1.5">
            <label className="text-xs font-bold text-[#15161B] uppercase tracking-wider">Sala</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-[#15161B] uppercase tracking-wider">Hora inicio</label>
              <input type="time" className={f2} value={form.horaInicio} onChange={e => setForm(f => ({ ...f, horaInicio: e.target.value }))} />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-[#15161B] uppercase tracking-wider">Duración</label>
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-bold text-[#15161B] uppercase tracking-wider">Días de la semana</label>
            <div className="flex items-center gap-2 flex-wrap">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-[#15161B] uppercase tracking-wider">Fecha inicio</label>
              <input type="date" className={f2} value={form.fechaInicio} onChange={e => setForm(f => ({ ...f, fechaInicio: e.target.value }))} />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-[#15161B] uppercase tracking-wider">Fecha fin</label>
              <input type="date" className={f2} value={form.fechaFin} onChange={e => setForm(f => ({ ...f, fechaFin: e.target.value }))} />
            </div>
          </div>
          <div className="space-y-1.5">
            <label className="text-xs font-bold text-[#15161B] uppercase tracking-wider">Aforo máximo</label>
          ...
          {estimatedCount > 0 && (
            <div className="rounded-xl bg-[#F1F1F6] px-4 py-3 flex items-center gap-2">
              <CalendarDays size={15} className="text-[#71727A] shrink-0" />
              <p className="text-sm font-semibold text-[#15161B]">Se crearán <span className="font-bold">{estimatedCount}</span> clases</p>
            </div>
          )}
        </div>
        <div className="flex gap-3 mt-6">
          <button onClick={onClose} className="flex-1 py-2.5 rounded-xl text-sm font-bold border border-[#ECECF1] text-[#71727A] hover:bg-gray-50 transition-colors">Cancelar</button>
          <button
            onClick={handleSubmit}
            disabled={form.diasSemana.length === 0 || estimatedCount === 0}
            className="flex-1 py-2.5 rounded-xl text-sm font-bold bg-[#15161B] text-white hover:bg-[#2A2B34] transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
          >
  ...
  return (
    <button
      ...
      className={cn(
        ...
        isSelected ? 'border-[#6355FF] ring-1 ring-[#6355FF]' : 'border-[#ECECF1] hover:border-[#D1D5DB] hover:shadow-sm',
    ...
    >
      ...
      <div className="text-center min-w-[52px] shrink-0">
        <p className="text-[15px] font-extrabold text-[#15161B] tabular-nums leading-none">{formatHora(s.inicio)}</p>
      ...
      {/* Info */}
      <div className="flex-1 min-w-0">
        <p className="text-[14px] font-bold text-[#15161B] truncate">
          {s.tipoClase.nombre}
      ...
      <div className="text-right shrink-0 flex items-center gap-2">
        ...
        <ChevronRight size={16} className="text-[#C7C8D0]" />
      </div>
    </button>
  );
}
    ...
    return (
      <div className="flex flex-col items-center justify-center text-center py-20 rounded-2xl border border-dashed border-[#E2E4EB] bg-white">
        <div className="w-14 h-14 rounded-2xl bg-[#EEEBFF] flex items-center justify-center mb-4">
          <CalendarDays size={26} className="text-[#6355FF]" />
        </div>
        <p className="text-[16px] font-bold text-[#15161B]">No hay clases esta semana</p>
        <p className="text-[13px] text-[#94A3B8] mt-1 mb-5">Crea la primera clase para empezar a llenar la agenda</p>
        <button onClick={onNueva} className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-[#15161B] text-white text-[13px] font-bold">
        ...
        return (
          <div key={str}>
            <div className="flex items-center gap-2 mb-2.5">
              <h3 className={cn('text-[15px] font-extrabold capitalize', isToday ? 'text-[#6355FF]' : 'text-[#15161B]')}>{label}</h3>
              {isToday && (
                <span className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full bg-[#6355FF] text-white">Hoy</span>
              )}
...
export default function Calendario() {
  ...
  const sesionesEnriquecidas = useMemo<SesionEnr[]>(() =>
    sesiones.map(s => ({
      ...s,
      tipoClase: tiposClase.find(t => t.id === s.tipoClaseId) ?? { nombre: '?', color: '#A2A3AC' },
  ...
  return (
    <div className="flex flex-col gap-4 h-full">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap shrink-0">
        <div>
          <h1 className="text-2xl font-bold text-[#15161B] tracking-tight">Calendario</h1>
          <p className="text-sm font-medium mt-0.5 capitalize text-[#71727A]">{mesLabel}</p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          {/* Week navigation */}
          <div className="flex items-center gap-1 bg-white border border-[#ECECF1] rounded-xl p-1">
            <button
              onClick={() => cambiarSemana(-1)}
              className="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-[#F1F1F6] text-[#71727A] transition-colors"
            >
              <ChevronLeft size={16} />
            </button>
            <button
              onClick={irAHoy}
              className="px-3 py-1 text-xs font-bold text-[#71727A] hover:text-[#15161B] transition-colors"
            >
              Hoy
            </button>
            <button
          ...
          <div className="relative">
            <div className="flex rounded-xl overflow-hidden bg-[#15161B]">
              <button
                onClick={() => openNueva()}
            ...
            {showNuevaMenu && (
              <>
                ...
                <div className="absolute right-0 top-full mt-1.5 z-20 bg-white border border-[#ECECF1] rounded-xl shadow-lg overflow-hidden min-w-[180px]">
                  <button
                    onClick={() => { setShowNuevaMenu(false); openNueva(); }}
                    className="w-full flex items-center gap-2.5 px-4 py-3 text-sm font-semibold text-[#15161B] hover:bg-[#F4F4F8] transition-colors text-left"
                  >
                    <Plus size={14} className="text-[#71727A]" />Clase única
                  </button>
                  <button
                    onClick={() => { setShowNuevaMenu(false); setShowRecurrentes(true); }}
                    className="w-full flex items-center gap-2.5 px-4 py-3 text-sm font-semibold text-[#15161B] hover:bg-[#F4F4F8] transition-colors text-left"
                  >
                    <RefreshCw size={14} className="text-[#71727A]" />Clases recurrentes
                  </button>
                </div>
              </>
            )}
      ...
      <Dialog open={showForm !== null} onOpenChange={open => !open && setShowForm(null)}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-lg font-bold text-[#15161B]">
          ...
          <div className="space-y-4 mt-2">
            ...
            {showForm === 'nueva' && (
              <div className="rounded-xl border border-[#ECECF1] p-4 space-y-3">
                <label className="flex items-center gap-3 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={form.repetir}
                    onChange={e => setForm(f => ({ ...f, repetir: e.target.checked }))}
                    className="w-4 h-4 rounded accent-[#6355FF]"
                  />
                  <span className="flex items-center gap-2 text-sm font-semibold text-[#15161B]">
                    <RefreshCw size={14} className="text-[#6355FF]" />
                    Repetir semanalmente
                  </span>
                </label>
                {form.repetir && (
                  <div className="flex items-center gap-3 pl-7">
                    <span className="text-sm text-[#71727A]">durante</span>
                    <input
                      type="number" min={2} max={52}
                      className="w-20 rounded-xl border border-[#ECECF1] bg-white px-3 py-2 text-sm font-medium text-[#15161B] focus:outline-none focus:border-[#A2A3AC] text-center"
                      value={form.repetirSemanas}
                      onChange={e => setForm(f => ({ ...f, repetirSemanas: Math.max(2, Number(e.target.value)) }))}
                    />
                    <span className="text-sm text-[#71727A]">semanas</span>
                  </div>
                )}
              </div>
            )}
          ...
          <div className="flex gap-3 mt-6">
            <button onClick={() => setShowForm(null)} className="flex-1 py-2.5 rounded-xl text-sm font-bold border border-[#ECECF1] text-[#71727A] hover:bg-gray-50 transition-colors">
              Cancelar
            </button>
            <button
              onClick={showForm === 'nueva' ? crearSesion : editarSesion}
              className="flex-1 py-2.5 rounded-xl text-sm font-bold text-white transition-opacity hover:opacity-90 bg-[#15161B]"
            >
              {showForm === 'nueva'
      ...
      {/* ── Toast ──────────────────────────────────────────────────────────────── */}
      {toast && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 bg-[#15161B] text-white px-5 py-3 rounded-2xl shadow-xl text-sm font-semibold animate-in fade-in slide-in-from-bottom-4">

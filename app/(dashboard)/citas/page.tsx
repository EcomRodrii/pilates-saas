...
const ESTADO_BADGE: Record<EstadoCita, { label: string; cls: string }> = {
  ...
  CANCELADA:   { label: 'Cancelada',    cls: 'bg-[#F1F1F6] text-[#71727A]' },
...
function FF({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="flex flex-col gap-1">
      <label className="text-sm font-medium text-[#15161B]">{label}</label>
      {children}
    </div>
  );
}
...
const inputCls =
  'w-full rounded-lg border border-[#ECECF1] px-3 py-2 text-sm text-[#15161B] outline-none focus:border-[#15161B] focus:ring-1 focus:ring-[#15161B] bg-white placeholder:text-[#A2A3AC]';
...
function CitaCard({
  ...
  return (
    <div
      className="bg-white border border-[#ECECF1] rounded-xl px-5 py-4 flex flex-col sm:flex-row sm:items-center gap-4 transition-shadow hover:shadow-sm"
    ...
    >
      ...
      <div className="flex items-center gap-3 min-w-0 flex-1">
        <div className="w-9 h-9 rounded-full bg-[#F1F1F6] flex items-center justify-center shrink-0">
          <span className="text-xs font-semibold text-[#71727A] uppercase">{socioInitials}</span>
        </div>
        <div className="min-w-0">
          <p className="text-sm font-medium text-[#15161B] truncate">{socioNombre}</p>
          <p className="text-xs text-[#71727A] truncate">{socioEmail}</p>
        </div>
      </div>

      {/* Instructor */}
      <div className="flex items-center gap-1.5 text-sm text-[#71727A] shrink-0">
      ...
      <div className="flex items-center gap-1 text-sm text-[#71727A] shrink-0">
        <Clock size={13} className="shrink-0" />
        <span>{mins} min</span>
      </div>

      {/* Price */}
      <span className="text-sm font-medium text-[#15161B] shrink-0 w-14 text-right">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-xl font-semibold text-[#15161B]">Citas</h1>
        ...
        <button
          ...
          className="inline-flex items-center gap-2 bg-[#15161B] text-white rounded-lg px-4 py-2.5 text-sm font-medium hover:bg-[#2A2B34] transition-colors shrink-0"
        >
          <Plus size={16} />
          Nueva cita
        </button>
      ...
      {/* Revenue banner */}
      <div className="bg-white border border-[#ECECF1] rounded-xl px-5 py-4 flex items-center justify-between">
        <div className="flex items-center gap-2 text-sm text-[#71727A]">
          <CheckCircle2 size={16} className="text-[#059669]" />
          <span>Ingresos completadas este mes</span>
        </div>
        <span className="text-xl font-bold text-[#15161B]">{ingresosMes} €</span>
      </div>

      {/* Tabs */}
      <div className="flex gap-1 bg-white border border-[#ECECF1] rounded-xl p-1 w-fit">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <div className="flex items-center gap-1.5 text-xs text-[#71727A]">
          <Filter size={13} />
        ...
        <button
          ...
          className={cn(
            ...
            filterInstructor === 'all'
              ...
              : 'bg-white border-[#ECECF1] text-[#71727A] hover:text-[#15161B]'
          )}
        >
          Todas
        </button>
      ...
      <div className="space-y-3">
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-[#71727A] text-sm">No hay citas que mostrar</p>
            <p className="text-[#A2A3AC] text-xs mt-1">
      ...
      <Dialog open={showModal} onOpenChange={setShowModal}>
        <DialogContent className="max-w-lg w-full">
          <DialogHeader>
            <DialogTitle className="text-lg font-semibold text-[#15161B]">Nueva cita</DialogTitle>
          </DialogHeader>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 mt-2">
            ...
            <div className="flex justify-end gap-2 pt-2">
              <button
                ...
                className="px-4 py-2 rounded-lg border border-[#ECECF1] bg-white text-sm font-medium text-[#71727A] hover:text-[#15161B] transition-colors"
              >
                Cancelar
              </button>
              <button
                type="submit"
                className="px-4 py-2 rounded-lg bg-[#15161B] text-white text-sm font-medium hover:bg-[#2A2B34] transition-colors"
              >
                Guardar cita
              </button>
            </div>

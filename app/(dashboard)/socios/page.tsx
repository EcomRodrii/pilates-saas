...
// ─── Shared style tokens ────────────────────────────────────────────────────
const inputCls =
  'w-full rounded-lg border border-[#F0E6E4] bg-white px-3 py-2 text-[13px] font-medium text-[#2B2429] focus:outline-none focus:border-[#B0A3A8] transition-colors placeholder:text-[#B0A3A8]';
...
function FF({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="space-y-1.5">
      <label className="text-[11px] font-semibold uppercase tracking-wide text-[#8B7D82]">
        {label}
      </label>
      {children}
    </div>
...
function StatCard({
  ...
  return (
    <div className="bg-white border border-[#F0E6E4] rounded-xl px-4 py-3 flex items-center gap-3 min-w-0">
      <div className="w-9 h-9 rounded-lg flex items-center justify-center shrink-0" style={{ backgroundColor: color + '1A' }}>
        <Icon size={16} style={{ color }} />
      </div>
      <div className="min-w-0">
        <p className="text-[22px] font-bold text-[#2B2429] leading-tight">{value}</p>
        <p className="text-[11px] text-[#8B7D82] truncate">{label}</p>
      </div>
    </div>
  );
}
...
function SortIcon({ active, dir }: { active: boolean; dir: SortDir }) {
  ...
  return dir === 'asc'
    ? <ChevronUp size={11} className="text-[#2B2429] ml-1 inline" />
    : <ChevronDown size={11} className="text-[#2B2429] ml-1 inline" />;
}
...
export default function Socios() {
  ...
  // Multi-step "nueva socia" contract flow
  ...
  function openEdit(s: Socio, e: React.MouseEvent) {
    e.stopPropagation();
  ...
  return (
    <div className="space-y-5 min-h-screen" style={{ backgroundColor: '#FDFBFA' }}>
      {/* ── Header ─────────────────────────────────────────────────────────── */}
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-xl font-bold text-[#2B2429] tracking-tight">Miembros</h1>
          <p className="text-[12px] text-[#B0A3A8] mt-0.5">
            Gestiona y haz seguimiento de todos tus miembros
          </p>
        </div>
        <button
          onClick={() => { setForm(emptyForm()); setShowForm('nueva'); }}
          className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-[13px] font-semibold text-white bg-[#2B2429] hover:bg-[#1F2937] transition-colors shrink-0 shadow-sm"
        >
          <Plus size={14} />
          Nuevo miembro
        </button>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <StatCard icon={Users} label="Total miembros" value={stats.total} color="#8B7D82" />
        <StatCard icon={UserCheck} label="Activas" value={stats.activas} color="#059669" />
        <StatCard icon={Bookmark} label="Con bono vigente" value={stats.conBono} color="#A05A6B" />
      ...
      <div className="space-y-3">
        ...
        <div className="relative">
          <Search size={14} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#B0A3A8]" />
          <input
            type="text"
            placeholder="Buscar por nombre, email o teléfono…"
            value={busqueda}
            onChange={(e) => setBusqueda(e.target.value)}
            className="w-full pl-10 pr-10 py-2.5 text-[13px] bg-white rounded-xl border border-[#F0E6E4] focus:outline-none focus:border-[#B0A3A8] transition-colors placeholder:text-[#B0A3A8] shadow-sm"
          />
          {busqueda && (
            <button
              onClick={() => setBusqueda('')}
              className="absolute right-3 top-1/2 -translate-y-1/2 text-[#B0A3A8] hover:text-[#8B7D82]"
            >
              <X size={13} />
            </button>
          )}
        ...
        <div className="flex items-center gap-2 flex-wrap">
            ...
            {SMART_FILTERS.map((f) => (
              <button
                ...
                className={cn(
                  ...
                  smartFilter === f.id
                    ? 'bg-[#2B2429] text-white border-[#2B2429] shadow-sm'
                    : 'bg-white text-[#8B7D82] border-[#F0E6E4] hover:border-[#B0A3A8] hover:text-[#4A3F44]',
                )}
              >
                {f.label}
              </button>
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            <span className="text-[11px] font-medium text-[#B0A3A8] hidden sm:inline">Ordenar:</span>
            <select
              value={sortKey}
              onChange={(e) => setSortKey(e.target.value as SortKey)}
              className="rounded-lg border border-[#F0E6E4] bg-white px-3 py-1.5 text-[12px] font-medium text-[#4A3F44] focus:outline-none appearance-none cursor-pointer"
            >
              {SORT_OPTIONS.map((o) => (
            ...
            <button
              ...
              className="p-1.5 rounded-lg border border-[#F0E6E4] bg-white hover:bg-[#FAF6F4] transition-colors"
              title={sortDir === 'asc' ? 'Ascendente' : 'Descendente'}
            >
              {sortDir === 'asc'
                ? <ChevronUp size={13} className="text-[#8B7D82]" />
                : <ChevronDown size={13} className="text-[#8B7D82]" />}
            </button>
          </div>
        </div>
      </div>
      ...
      {/* ── Bulk action bar ─────────────────────────────────────────────────── */}
      {selected.size > 0 && (
        <div className="flex items-center gap-2 px-4 py-3 bg-[#2B2429] text-white rounded-xl shadow-lg">
          <span className="text-[12px] font-medium text-[#B0A3A8] mr-1">
          ...
          <button
            ...
            className="ml-1 text-[#B0A3A8] hover:text-white transition-colors p-1"
          >
            <X size={14} />
          </button>
        </div>
      ...
      {/* ── Table ──────────────────────────────────────────────────────────── */}
      <div className="bg-white rounded-xl border border-[#F0E6E4] overflow-x-auto shadow-sm">
        {lista.length === 0 ? (
          /* Empty state */
          <div className="flex flex-col items-center justify-center py-20 px-6 text-center">
            <div className="w-16 h-16 rounded-2xl bg-[#F5EFEC] flex items-center justify-center mb-4">
              <Users size={28} className="text-[#B0A3A8]" />
            </div>
            <h3 className="text-[15px] font-semibold text-[#2B2429] mb-1">
              {busqueda || smartFilter !== 'todas'
                ? 'No hay resultados'
                : 'Aún no hay miembros'}
            </h3>
            <p className="text-[13px] text-[#8B7D82] mb-5 max-w-xs">
              {busqueda || smartFilter !== 'todas'
            ...
            {!busqueda && smartFilter === 'todas' && (
              <button
                onClick={() => { setForm(emptyForm()); setShowForm('nueva'); }}
                className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-[13px] font-semibold text-white bg-[#2B2429] hover:bg-[#1F2937] transition-colors"
              >
                <Plus size={14} />
            ...
            {(busqueda || smartFilter !== 'todas') && (
              <button
                ...
                className="text-[12px] font-medium text-[#8B7D82] underline underline-offset-2 hover:text-[#4A3F44] transition-colors"
              >
                Limpiar filtros
              </button>
            )}
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-[#F5EFEC] bg-[#FAFAFA]">
                {/* Checkbox */}
                <th className="pl-4 pr-2 py-3 w-9">
                  <input
                    ...
                    className="rounded border-[#D1D5DB] accent-[#2B2429] cursor-pointer"
                  />
                </th>
                {/* Socia */}
                <th className="text-left px-4 py-3">
                  <button
                    ...
                    className="flex items-center text-[11px] font-semibold uppercase tracking-wide text-[#B0A3A8] hover:text-[#2B2429] transition-colors"
                  >
                    Miembro
                ...
                <th className="text-left px-4 py-3 hidden sm:table-cell">
                  <span className="text-[11px] font-semibold uppercase tracking-wide text-[#B0A3A8]">
                    Plan actual
                  </span>
                </th>
                {/* Sesiones restantes */}
            ...
            <tbody className="divide-y divide-[#FAF6F4]">
              {lista.map((s) => {
                ...
                let sesBg = '#D1FAE5';
                if (sesRest != null) {
                  if (sesRest <= 0) { sesColor = '#DC2626'; sesBg = '#FEE2E2'; }
                ...
                if (!s.activo) {
                  estadoBadge = (
                    <span className="inline-flex items-center gap-1 text-[11px] font-medium px-2 py-0.5 rounded-md bg-[#F5EFEC] text-[#8B7D82]">
                      <span className="w-1.5 h-1.5 rounded-full bg-[#B0A3A8]" />
                      Inactiva
                    </span>
                  );
                } else if (expirado) {
                ...
                return (
                  <tr
                    ...
                    className={cn(
                      'hover:bg-[#FAF6F4] transition-colors group cursor-pointer',
                      isSelected && 'bg-[#F7EDEF]',
                    )}
                  >
                    {/* Checkbox */}
                    ...
                    <td className="px-4 py-3.5">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          <p className="text-[13px] font-semibold text-[#2B2429] truncate">
                            {s.nombre} {s.apellidos}
                          </p>
                          <p className="text-[11px] text-[#B0A3A8] truncate">{s.email}</p>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-3.5 hidden sm:table-cell">
                      {plan ? (
                        <div>
                          <p className="text-[12px] font-medium text-[#4A3F44]">{plan.nombre}</p>
                          {sus?.estado === 'PAUSADA' && (
                    ...
                    {/* Última asistencia */}
                    <td className="px-4 py-3.5 text-[12px] text-[#8B7D82] hidden lg:table-cell">
                    ...
                    <td className="px-4 py-3.5" onClick={(e) => e.stopPropagation()}>
                      <div className="flex items-center justify-end gap-0.5 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                        <button
                          ...
                          className="p-1.5 rounded-md hover:bg-[#F5EFEC] transition-colors"
                          title="Editar"
                        >
                          <Pencil size={13} className="text-[#8B7D82]" />
                        </button>
                        <button
                        ...
                        >
                          {s.activo
                            ? <UserX size={13} className="text-[#8B7D82]" />
        ...
        {/* Table footer */}
        {lista.length > 0 && (
          <div className="px-5 py-3 border-t border-[#F5EFEC] bg-[#FAFAFA]">
            <p className="text-[11px] text-[#B0A3A8]">
      ...
      >
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle className="text-base font-semibold text-[#2B2429] flex items-center gap-2">
              {showForm === 'nueva' && formStep === 2 && (
                <button onClick={() => setFormStep(1)} className="p-0.5 rounded hover:bg-[#F5EFEC]">
                  <ArrowLeft size={15} className="text-[#8B7D82]" />
                </button>
              )}
              {showForm === 'nueva'
              ...
              {[{ n: 1, label: 'Datos' }, { n: 2, label: 'Contrato' }].map(({ n, label }) => (
                <div key={n} className="flex items-center gap-1.5">
                  <div className={cn(
                    ...
                    formStep >= n ? 'bg-[#2B2429] text-white' : 'bg-[#F0E6E4] text-[#B0A3A8]',
                  )}>
                    {formStep > n ? <CheckCircle2 size={11} /> : n}
                  </div>
                  <span className={cn(
                    'text-[11px] font-medium',
                    formStep >= n ? 'text-[#2B2429]' : 'text-[#B0A3A8]',
                  )}>{label}</span>
                  {n < 2 && <div className="w-6 h-px bg-[#F0E6E4] mx-0.5" />}
                </div>
              ))}
            </div>
          )}

          {/* ── Step 1: Datos de la socia ─── */}
            ...
            <div className="space-y-3.5 mt-2">
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  <label className="text-[11px] font-semibold uppercase tracking-wide text-[#8B7D82] flex items-center gap-1.5">
                    <FileText size={11} />
                    Política de privacidad y condiciones
                  </label>
                  {!scrolledToBottom && (
                    <span className="text-[10px] text-[#B0A3A8]">Desplaza hasta el final ↓</span>
                  )}
                  {scrolledToBottom && (
                ...
                <div
                  ...
                  className="h-52 overflow-y-auto rounded-lg border border-[#F0E6E4] bg-[#FAF6F4] p-3 text-[11px] text-[#4A3F44] leading-relaxed whitespace-pre-wrap font-mono"
                >
              ...
              <label className={cn(
                ...
                aceptado ? 'border-[#059669] bg-[#F0FDF4]' : 'border-[#F0E6E4] bg-white hover:border-[#B0A3A8]',
              )}>
                <input
                  type="checkbox"
                  checked={aceptado}
                ...
                />
                <span className="text-[12px] text-[#4A3F44] leading-snug">
              ...
              <FF label="Firma (nombre completo como firma digital)">
                <div className="relative">
                  <PenLine size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#B0A3A8]" />
                  <input
              ...
              {/* Acceptance summary */}
              {aceptado && firma.trim() && (
                <div className="flex items-start gap-2 px-3 py-2.5 bg-[#F0FDF4] border border-[#BBF7D0] rounded-lg">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-[#8B7D82] mt-0.5">
                      Firmado digitalmente por <span className="font-medium text-[#4A3F44]">{firma.trim()}</span> —{' '}
          ...
          <div className="flex gap-2 mt-5">
            <button
              onClick={resetModal}
              className="flex-1 py-2 rounded-xl text-[13px] font-medium border border-[#F0E6E4] text-[#8B7D82] hover:bg-[#FAF6F4] transition-colors"
            >
              Cancelar
            </button>
            {showForm === 'nueva' && formStep === 1 ? (
              <button
                ...
                className="flex-1 py-2 rounded-xl text-[13px] font-medium text-white bg-[#2B2429] disabled:opacity-40 hover:bg-[#1F2937] transition-colors"
              >
                Siguiente — Contrato
              </button>
            ) : (
      ...
      >
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle className="text-base font-semibold text-[#2B2429]">
          ...
          <div className="flex flex-col items-center text-center gap-3 py-2">
            ...
            <div className="flex gap-2 w-full">
              ...
              <button
                ...
                className="flex-1 py-2 rounded-xl text-[13px] font-medium text-white bg-[#DC2626] hover:bg-[#B91C1C] transition-colors"
              >
                Eliminar
              </button>
            </div>

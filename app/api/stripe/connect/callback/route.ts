...
export async function GET(req: NextRequest) {
  ...
  const redirigir = (query: string) => {
    const res = NextResponse.redirect(`${appUrl}/configuracion?tab=cobros&${query}`);
    ...
    try {
      ...
      const { error: errLog } = admin
        ? await admin.from('actividad_reciente').insert({
          ...
          socio_id: null, enlace: '/configuracion?tab=cobros#integracion-stripe', creado_en: new Date().toISOString(), actor_nombre: null,
        })
        : { error: null };

...
// diferencias: PKCE (el code_verifier sale de la cookie HttpOnly del flujo,
// nunca del `state` de la URL — ver lib/oauth-state.ts) y la creación de la
// lista de sincronización en el primer connect.
export async function GET(req: NextRequest) {
  const appUrl = process.env.NEXT_PUBLIC_APP_URL ?? 'http://localhost:3001';
  // H-1: la cookie del flujo es de un solo uso — se borra en TODAS las salidas.
  const redirigir = (query: string) => {
    const res = NextResponse.redirect(`${appUrl}/configuracion?tab=conexiones&${query}`);
    borrarCookieOAuth(res, 'klaviyo');
    return res;
  };
  if (!isKlaviyoConfigurado()) {
    return redirigir('klaviyo_error=Klaviyo%20no%20configurado');
  }

  const code = req.nextUrl.searchParams.get('code');
  const oauthError = req.nextUrl.searchParams.get('error_description') ?? req.nextUrl.searchParams.get('error');
  if (oauthError) {
    return redirigir(`klaviyo_error=${encodeURIComponent(oauthError)}`);
  }

  // C-8: studioId desde el `state` FIRMADO, no de un id en claro — evita
  // injertar los tokens de Klaviyo en un estudio ajeno (CSRF de binding).
  // H-1: y solo junto a la cookie que se fijó en ESTE navegador al pedirlo.
  const verificado = verificarEstadoOAuth(
    req.nextUrl.searchParams.get('state'), 'klaviyo', Date.now(),
    req.cookies.get(nombreCookieOAuth('klaviyo'))?.value,
  );
  if (!code || !verificado || !verificado.codeVerifier) {
    return redirigir('klaviyo_error=Estado%20de%20conexi%C3%B3n%20inv%C3%A1lido%20o%20caducado');
  }
  ...
  try {
    ...
    return redirigir('klaviyo_connected=1');
  } catch (err) {
    console.error('[integrations/klaviyo/callback]', err instanceof Error ? err.message : err);
    return redirigir(`klaviyo_error=${encodeURIComponent('No se pudo completar la conexión con Klaviyo. Inténtalo de nuevo.')}`);
  }
}

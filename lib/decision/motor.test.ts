...
test('fixture del mockup: Laura + Marta + martes 19h + 2 pagos → Prioridades = las 3 de mayor score', () => {
  ...
  const snapshot: SnapshotEstudio = {
    ...
    tiposClase: [], instructores: [], automationLogs: [], campanas: [], sustituciones: [], instructorTarifas: new Map(), contexto: { nSociasActivas: 0, antiguedadDatosDias: 999, cadenaId: null, nSedesCadena: 1 },
  };
...
test('flagsEspecialistas: FINANZAS=false apaga ese especialista sin tocar los demás', () => {
  const snapshot: SnapshotEstudio = {
    ...
    tiposClase: [], instructores: [], automationLogs: [], campanas: [], sustituciones: [], instructorTarifas: new Map(),

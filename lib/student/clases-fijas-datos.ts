...
export async function pedirClaseFija(slug: string, studioId: string, claseFijaId: string, duracionMeses: number): Promise<ResultadoPedirClaseFija> {
  try {
    const auth = await portalAuthHeader();
    const res = await fetch('/api/public/plaza-fija', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', ...auth },
      body: JSON.stringify({ accion: 'solicitar_clase_fija', studioId, claseFijaId, duracionMeses }),
    });
    if (res.status === 401) return { ok: false, error: 'Tu sesión ha caducado: vuelve a entrar para pedirla.', sesionCaducada: true };
    const d = (await res.json().catch(() => null)) as { solicitudId?: unknown; error?: unknown } | null;
    ...
    return { ok: true, solicitudId: typeof d?.solicitudId === 'string' ? d.solicitudId : null };
  } catch {

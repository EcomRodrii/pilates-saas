...
> **Nota 2026-09-04.** La cifra «0/56» era verdad al diseñar y ya no lo es: desde I-3 el calendario crea las recurrentes como serie (`serie_id`), y el importador también. D1 se mantiene (la plaza sigue anclada al slot, no a la serie), pero el slot **hay que moverlo cuando la clase se mueve** — nada lo hacía y la plaza dejaba de materializar en silencio. Desde la migración `20260904153000`, `editar_serie_desde` mueve las plazas ancladas al slot viejo (hora/sala/tipo; nunca el día, que la RPC no cambia). Arrastrar UNA clase no mueve la plaza a propósito (es una excepción puntual); ese caso lo cubren la bandeja «Para hoy» (plaza sin clase en su horario, `lib/plazas-fijas-slot.ts`) y la edición del slot desde la ficha de la socia.

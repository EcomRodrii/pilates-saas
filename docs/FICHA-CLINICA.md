...
Dato de salud → acceso restringido. **PROPIETARIO** ve el detalle clínico de todas las socias; **INSTRUCTOR** solo el de **sus alumnas** (reserva no cancelada en una clase suya, o cita con ella, entre hace 30 días y dentro de 30 — `instructora_atiende_socia()` en la RLS y `lib/datos-salud/acceso-instructora.ts` en las rutas service-role; migr `20260913214116`); **RECEPCIÓN/MANAGER** ven solo el **color del semáforo** (no el motivo ni las condiciones). Todo exige además **consentimiento de salud vigente**, que se registra y se retira solo por servidor (`/api/socios/[id]/consentimiento-salud`, `/api/public/consentimiento-salud/revocar`) con texto derivado en servidor e historial en `consentimientos_salud_eventos` (migr `20260913214142`). Retirarlo **bloquea** los datos (dejan de verse), no los borra — ⚠️ decisión legal pendiente.

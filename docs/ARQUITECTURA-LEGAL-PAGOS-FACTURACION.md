...
### Riesgo activo a corregir cuanto antes (no es solo documentación)

`components/cobros/panel-facturas.tsx` sigue mostrando el banner **"Verifactu — Próximamente / Integración con AEAT en desarrollo"** inmediatamente encima del bloque que, si la factura ya tiene huella, muestra el QR real y dice "Sistema de facturación verificable". El banner no se ha tocado desde antes de que existiera ninguno de los PRs de Verifactu (#212, #290, #313, #510). **Es un mensaje objetivamente falso en producción ahora mismo**: le dice al usuario que algo no existe cuando el propio bloque de debajo demuestra que sí. Esto no es un matiz de roadmap — es una corrección de una línea que debería hacerse independientemente de este documento.
...
- Corregir el banner falso "Verifactu — Próximamente" en `panel-facturas.tsx`.
...
1. **El banner falso de Verifactu en producción** (§2) — el hallazgo más urgente y accionable de todo el documento, y el único que no requiere ninguna decisión de producto: es una corrección de una línea.
